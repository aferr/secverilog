...
task syscall;
   ...
   begin
      ...
      /*if (op_code == 4013) begin 
        addr = CPU.regfile.RAM[4];
      end*/
	...
	begin
	 ...
	 // simulator) is more recent calculate index of addr 
      ...
      // for the time syscall, we also need to invalidate the data cache in the
      // processor for a similar reason
      /*if (op_code == 4013 && addr != 0) begin 
         $display ("calling time");
         $display ("addr is %x", addr);

         addrIndex = addr[`D_INDEX];
         if ((CPU.MEMcontr.dc_tmem.mem0[addrIndex] == addr[`D_TAG]) 
           & (CPU.MEMcontr.dc_tmem.mem0[addrIndex][`D_VALID])) begin
             CPU.MEMcontr.dc_tmem.mem0[addrIndex][`D_VALID] = 1'b0;
         end
         if ((CPU.MEMcontr.dc_tmem.mem1[addrIndex] == addr[`D_TAG]) 
           & (CPU.MEMcontr.dc_tmem.mem1[addrIndex][`D_VALID])) begin
             CPU.MEMcontr.dc_tmem.mem1[addrIndex][`D_VALID] = 1'b0;
         end
         if ((CPU.MEMcontr.dc_tmem.mem2[addrIndex] == addr[`D_TAG]) 
           & (CPU.MEMcontr.dc_tmem.mem2[addrIndex][`D_VALID])) begin
             CPU.MEMcontr.dc_tmem.mem2[addrIndex][`D_VALID] = 1'b0;
         end
         if ((CPU.MEMcontr.dc_tmem.mem3[addrIndex] == addr[`D_TAG]) 
           & (CPU.MEMcontr.dc_tmem.mem3[addrIndex][`D_VALID])) begin
             CPU.MEMcontr.dc_tmem.mem3[addrIndex][`D_VALID] = 1'b0;
         end
      end*/
