...
bool NetExpr::set_width(unsigned w, bool) {
  cerr << get_fileline() << ": internal warning:  " << typeid(*this).name()
       << ": set_width(unsigned) "
...
bool NetEBinary::set_width(unsigned w, bool) {
  ...
  default:
    expr_width(left_->expr_width() > right_->expr_width()
                   ? left_->expr_width()
                   : right_->expr_width());
    cerr << "NetEBinary::set_width(): Using default for " << op_ << "." << endl;
    flag = false;
 ...
 * fixed width of 1.
 */
bool NetEBComp::set_width(unsigned w, bool) { return (w == 1); }
 ...
 */
bool NetEBDiv::set_width(unsigned w, bool) { return w == expr_width(); }
...
bool NetEConst::set_width(unsigned w, bool last_chance) {
  ...
  if (w > value_.len()) {
    ...
    verinum::V pad_bit =
        value_.has_sign() ? value_[value_.len() - 1] : verinum::V0;
 ...
 * value.
 */
bool NetEConstParam::set_width(unsigned w, bool) { return w == expr_width(); }
...
#endif
bool NetEParam::set_width(unsigned, bool) { return false; }
...
bool NetESFunc::set_width(unsigned w, bool) { return w == expr_width(); }
...
bool NetETernary::set_width(unsigned w, bool last_chance) {
  ...
  if (true_val_->expr_width() < false_val_->expr_width())
    true_val_ = pad_to_width(true_val_, false_val_->expr_width(), *this);
  if (false_val_->expr_width() < true_val_->expr_width())
    false_val_ = pad_to_width(false_val_, true_val_->expr_width(), *this);
 ...
 */
bool NetEUReduce::set_width(unsigned w, bool) { return w == 1; }
