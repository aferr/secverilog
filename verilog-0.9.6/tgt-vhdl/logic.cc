...
static vhdl_expr *inputs_to_expr(vhdl_scope *scope, vhdl_binop_t op,
  ...
  vhdl_binop_expr *gate = new vhdl_binop_expr(op, vhdl_type::std_logic());
...
static void comb_udp_logic(vhdl_arch *arch, ivl_net_logic_t log) {
  ...
  vhdl_signal_decl *tmp_decl = new vhdl_signal_decl(ss.str().c_str(), tmp_type);
  ...
  ss.str("");
  ss << "Input to " << ivl_logic_basename(log) << " " << ivl_udp_name(udp)
     << " UDP";
  tmp_decl->set_comment(ss.str());

  vhdl_var_ref *tmp_ref = new vhdl_var_ref(tmp_decl->get_name().c_str(), NULL);
...
static vhdl_expr *translate_logic_inputs(vhdl_scope *scope,
                                         ivl_net_logic_t log) {
