...
int indent(int level) { return level + VHDL_INDENT; }
...
void vhdl_element::set_comment(std::string comment) { comment_ = comment; }
...
void vhdl_element::operator delete(void *ptr) {
  ...
  vector<vhdl_element *>::iterator it = find(
      allocated_.begin(), allocated_.end(), static_cast<vhdl_element *>(ptr));
...
size_t vhdl_element::total_allocated() { return total_alloc_; }
