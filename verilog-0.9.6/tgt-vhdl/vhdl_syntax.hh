...
public:
  ...
  vhdl_binop_expr(vhdl_expr *left, vhdl_binop_t op, vhdl_expr *right,
                  vhdl_type *type);
  ~vhdl_binop_expr();
...
enum vhdl_unaryop_t { VHDL_UNARYOP_NOT, VHDL_UNARYOP_NEG };

class vhdl_unaryop_expr : public vhdl_expr {
public:
  vhdl_unaryop_expr(vhdl_unaryop_t op, vhdl_expr *operand, vhdl_type *type)
...
enum time_unit_t { TIME_UNIT_PS, TIME_UNIT_NS, TIME_UNIT_US, TIME_UNIT_MS };
...
public:
  vhdl_assert_stmt(const char *reason) : reason_(reason) {}
...
public:
  ...
  virtual assign_type_t assignment_type() const {
    assert(false);
    return ASSIGN_BLOCK;
  }
...
public:
  vhdl_param_decl(const char *name, vhdl_type *type) : vhdl_decl(name, type) {}
  void emit(std::ostream &of, int level) const;
...
public:
  vhdl_port_decl(const char *name, vhdl_type *type, vhdl_port_mode_t mode)
