 ...
 */
static string visible_nexus_signal_name(nexus_private_t *priv,
                                        vhdl_scope *scope, unsigned *pin) {
...
void draw_nexus(ivl_nexus_t nexus) {
  ...
  for (int i = 0; i < nptrs; i++) {
    ...
    if ((log = ivl_nexus_ptr_log(nexus_ptr))) {
      ...
      if (visible_nexus(priv, vhdl_scope)) {
        ...
        vhdl_type *type = vhdl_type::type_for(ivl_logic_width(log), false);
  ...
  if (ndrivers == 0) {
    ...
    if (def) {
      if (width > 1)
        priv->const_driver = new vhdl_bit_spec_expr(vhdl_type::std_logic(),
...
string make_safe_name(ivl_signal_t sig) {
  ...
  const char *vhdl_reserved[] = {"abs",          "access",     "after",
                                 "alias",        "all",        "and",
...
static void declare_one_signal(vhdl_entity *ent, ivl_signal_t sig,
  ...
  if (dimensions > 0) {
    ...
    vhdl_type *array_type = vhdl_type::array_of(base_type, type_name, msb, lsb);
  ...
  case IVL_SIP_OUTPUT: {
    ...
    if (ivl_signal_type(sig) == IVL_SIT_REG) {
      ...
      ent->get_arch()->add_stmt(
          new vhdl_cassign_stmt(new vhdl_var_ref(name.c_str(), NULL),
...
static void map_signal(ivl_signal_t to, vhdl_entity *parent,
  ...
  if (visible_nexus(priv, arch_scope)) {
    vhdl_var_ref *ref =
        nexus_to_var_ref(parent->get_arch()->get_scope(), nexus);
...
static int draw_function(ivl_scope_t scope, ivl_scope_t parent) {
  ...
  for (int i = 0; i < nports; i++) {
    ...
    case IVL_SIP_OUTPUT:
      ...
      func->get_scope()->add_decl(new vhdl_var_decl(signame, sigtype));
      break;
    default:
  ...
  for (int i = 0; i < nsigs; i++) {
    ...
    if (ivl_signal_port(sig) == IVL_SIP_NONE) {
      vhdl_type *sigtype = vhdl_type::type_for(ivl_signal_width(sig),
  ...
  ent->get_arch()->get_scope()->add_forward_decl(new vhdl_forward_fdecl(func));
  ...
  if (ivl_scope_type(scope) == IVL_SCT_MODULE) {
    ...
    for (int i = 0; i < nsigs; i++) {
      ...
      for (unsigned j = ivl_signal_array_base(sig);
           j < ivl_signal_array_count(sig); j++) {
        ...
        if (ivl_signal_port(sn->sig) != IVL_SIP_INPUT) {
          for (list<ivl_signal_t>::const_iterator it = sn->connect.begin();
               it != sn->connect.end(); ++it) {
            vhdl_type *rtype = vhdl_type::type_for(ivl_signal_width(sn->sig),
                                                   ivl_signal_signed(sn->sig));
            vhdl_type *ltype = vhdl_type::type_for(ivl_signal_width(*it),
...
extern "C" int draw_hierarchy(ivl_scope_t scope, void *_parent) {
  if (ivl_scope_type(scope) == IVL_SCT_MODULE && _parent) {
    ...
    ostringstream ss;
    ss << "Generated from instantiation at " << ivl_scope_file(scope) << ":"
