...
static vhdl_expr *correct_signedness(vhdl_expr *vhd_e, ivl_expr_t vl_e) {
  ...
  if (vhd_e->get_type()->get_name() == VHDL_TYPE_UNSIGNED && should_be_signed) {
    // operand->print();
...
static vhdl_expr *translate_unary(ivl_expr_t e) {
  ...
  case '!':
  case '~':
    return new vhdl_unaryop_expr(VHDL_UNARYOP_NOT, operand,
                                 new vhdl_type(*operand->get_type()));
  case '-':
    operand = change_signedness(operand, true);
    return new vhdl_unaryop_expr(VHDL_UNARYOP_NEG, operand,
                                 new vhdl_type(*operand->get_type()));
  case 'N': // NOR
 ...
 */
static vhdl_expr *translate_power(ivl_expr_t e, vhdl_expr *lhs,
                                  vhdl_expr *rhs) {
  vhdl_type integer(VHDL_TYPE_INTEGER);
  ...
  vhdl_type *type  = ivl_expr_signed(e) ? vhdl_type::nsigned(width)
...
static vhdl_expr *translate_binary(ivl_expr_t e) {
  ...
  bool vectorop = (ltype == VHDL_TYPE_SIGNED || ltype == VHDL_TYPE_UNSIGNED) &&
  ...
  if (vectorop) {
    ...
    if (actual_width != result_width) {
      // result->print();
      // std::cout << "^ should be " << result_width << " but is " <<
      // actual_width << std::endl;
    }
  }
...
template <class T> static T *translate_parms(T *t, ivl_expr_t e) {
...
static vhdl_expr *translate_ufunc(ivl_expr_t e) {
  ...
  for (int i = 0; i < nparams; i++) {
    ...
    vhdl_type *param_type  = vhdl_type::type_for(
        ivl_signal_width(param_sig), ivl_signal_signed(param_sig) != 0);
...
static vhdl_expr *translate_ternary(ivl_expr_t e) {
  ...
  vhdl_fcall *fcall = new vhdl_fcall(support_function::function_name(sf),
...
vhdl_expr *translate_expr(ivl_expr_t e) {
  ...
  case IVL_EX_REALNUM:
    error("No VHDL translation for real expression at %s:%d", ivl_expr_file(e),
          ivl_expr_lineno(e));
    return NULL;
  default:
...
vhdl_expr *translate_time_expr(ivl_expr_t e) {
  ...
  if (time->get_type()->get_name() != VHDL_TYPE_TIME) {
    ...
    return new vhdl_binop_expr(time, VHDL_BINOP_MULT, ns1, vhdl_type::time());
