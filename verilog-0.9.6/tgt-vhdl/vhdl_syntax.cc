...
vhdl_scope::vhdl_scope() : parent_(NULL), init_(false), sig_assign_(true) {}
...
void vhdl_scope::add_decl(vhdl_decl *decl) { decls_.push_back(decl); }

void vhdl_scope::add_forward_decl(vhdl_decl *decl) { decls_.push_front(decl); }
...
vhdl_entity::vhdl_entity(const string &name, vhdl_arch *arch, int depth__)
    : depth(depth__), name_(name), arch_(arch), time_unit_(TIME_UNIT_NS) {
  arch->get_scope()->set_parent(&ports_);
}

vhdl_entity::~vhdl_entity() {}

void vhdl_entity::add_port(vhdl_port_decl *decl) { ports_.add_decl(decl); }
...
void vhdl_arch::add_stmt(vhdl_conc_stmt *stmt) { stmts_.push_back(stmt); }
...
vhdl_component_decl::vhdl_component_decl(const char *name) : vhdl_decl(name) {}
...
vhdl_component_decl *vhdl_component_decl::component_decl_for(vhdl_entity *ent) {
  ...
  vhdl_component_decl *decl = new vhdl_component_decl(ent->get_name().c_str());
...
void vhdl_expr_list::add_expr(vhdl_expr *e) { exprs_.push_back(e); }
...
vhdl_const_bits::vhdl_const_bits(const char *value, int width, bool issigned,
    ...
    : vhdl_expr(issigned ? vhdl_type::nsigned(width)
                ...
                true),
      qualified_(qualify), signed_(issigned) {
...
static bool is_meta_bit(char c) { return c != '1' && c != '0'; }
...
void vhdl_const_bits::emit(std::ostream &of, int level) const {
  ...
  if ((!signed_ || ival >= 0) && !has_meta_bits() && bits <= 64 &&
      bits % 4 == 0) {
...
void vhdl_binop_expr::add_expr(vhdl_expr *e) { operands_.push_back(e); }
...
void vhdl_with_select_stmt::add_default(vhdl_expr *value) { others_ = value; }
