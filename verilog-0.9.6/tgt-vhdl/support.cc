...
const char *support_function::function_name(support_function_t type) {
  ...
  case SF_LOGIC_TO_INTEGER:
    return "Logic_To_Integer";
  case SF_SIGNED_TO_LOGIC:
    return "Signed_To_Logic";
  case SF_UNSIGNED_TO_LOGIC:
    return "Unsigned_To_Logic";
  default:
    assert(false);
  }
  return "Invalid";
...
void support_function::emit_reduction(std::ostream &of, int level,
  ...
  of << "(X : std_logic_vector) return std_logic is" << nl_string(indent(level))
     << "variable R : std_logic := '" << unit << "';" << nl_string(level)
     << "begin" << nl_string(indent(level)) << "for I in X'Range loop"
     << nl_string(indent(indent(level))) << "R := X(I) " << op << " R;"
     << nl_string(indent(level)) << "end loop;" << nl_string(indent(level))
     << "return R;";
}
...
void support_function::emit(std::ostream &of, int level) const {
  ...
  switch (type_) {
  case SF_UNSIGNED_TO_BOOLEAN:
    of << "(X : unsigned) return Boolean is" << nl_string(level) << "begin"
       << nl_string(indent(level)) << "return X /= To_Unsigned(0, X'Length);";
    break;
  case SF_SIGNED_TO_BOOLEAN:
    of << "(X : signed) return Boolean is" << nl_string(level) << "begin"
       << nl_string(indent(level)) << "return X /= To_Signed(0, X'Length);";
    break;
  case SF_BOOLEAN_TO_LOGIC:
    of << "(B : Boolean) return std_logic is" << nl_string(level) << "begin"
       << nl_string(indent(level)) << "if B then"
       << nl_string(indent(indent(level))) << "return '1';"
       << nl_string(indent(level)) << "else" << nl_string(indent(indent(level)))
       << "return '0';" << nl_string(indent(level)) << "end if;";
    break;
  case SF_UNSIGNED_TO_LOGIC:
    of << "(X : unsigned) return std_logic is" << nl_string(level) << "begin"
       << nl_string(indent(level)) << "return X(0);";
    break;
  case SF_SIGNED_TO_LOGIC:
    of << "(X : signed) return std_logic is" << nl_string(level) << "begin"
       << nl_string(indent(level)) << "return X(0);";
    break;
  case SF_REDUCE_OR:
  ...
  case SF_TERNARY_UNSIGNED:
    ...
    break;
  case SF_LOGIC_TO_INTEGER:
    of << "(X : std_logic) return integer is" << nl_string(level) << "begin"
