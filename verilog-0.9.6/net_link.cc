...
const Nexus *Link::nexus() const { return nexus_; }
...
void Link::drive0(Link::strength_t str) { drive0_ = str; }

void Link::drive1(Link::strength_t str) { drive1_ = str; }

Link::strength_t Link::drive0() const { return drive0_; }

Link::strength_t Link::drive1() const { return drive1_; }

void Link::set_init(verinum::V val) { init_ = val; }

verinum::V Link::get_init() const { return init_; }
...
Link *Link::next_nlink() { return next_; }

const Link *Link::next_nlink() const { return next_; }
...
verinum::V Nexus::get_init() const {
  ...
  for (Link *cur = list_; cur; cur = cur->next_) {
    ...
    if ((cur->get_dir() == Link::PASSIVE) && (cur->get_init() != verinum::Vz))
      return cur->get_init();
  }
...
Link *Nexus::first_nlink() { return list_; }

const Link *Nexus::first_nlink() const { return list_; }

ivl_nexus_t Nexus::t_cookie() const { return t_cookie_; }
...
unsigned NexusSet::count() const { return nitems_; }
