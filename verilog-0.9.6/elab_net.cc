...
NetNet *PEIdent::make_implicit_net_(Design *des, NetScope *scope) const {
  ...
  NetNet *sig = new NetNet(scope, peek_tail_name(path_), nettype, 1);
  sig->set_line(*this);
  ...
  if (warn_implicit) {
    cerr << get_fileline()
         ...
         << scope_path(scope) << "." << peek_tail_name(path_) << "." << endl;
  }
    ...
    if (name_tail.index.size() > sig->array_dimensions()) {
      ...
      if (mval == 0) {
        cerr << get_fileline() << ": error: Index of " << path_
             << " needs to be constant in this context." << endl;
        cerr << get_fileline()
             << ":      : Index expression is: " << *index_tail.msb << endl;
        cerr << get_fileline()
             << ":      : Context scope is: " << scope_path(scope) << endl;
        des->errors += 1;
        return false;
      }
      assert(mval);
      ...
      if (midx >= (long)sig->vector_width()) {
        cerr << get_fileline() << ": error: Index " << sig->name() << "["
             << mval->as_long() << "] is out of range." << endl;
        des->errors += 1;
        midx = 0;
      }
      lidx = midx;
    ...
    if (name_tail.index.size() > sig->array_dimensions()) {
      if (sig->get_scalar()) {
        ...
        if (sig->data_type() == IVL_VT_REAL)
          ...
          cerr << "scalar";
        cerr << " array word: " << sig->name() << "[" << widx_val << "]"
             << endl;
        des->errors += 1;
        return 0;
      }
...
NetNet *PEIdent::elaborate_port(Design *des, NetScope *scope) const {
  ...
  NetNet *tmp = new NetNet(scope, scope->local_symbol(), NetNet::WIRE, swid);
