...
static PLI_INT32 sys_dumpflush_calltf(PLI_BYTE8 *name) {
  if (dump_file)
    fstWriterFlushContext(dump_file);
...
static void scan_item(unsigned depth, vpiHandle item, int skip) {
  ...
  case vpiNet:
    ...
    if (vcd_names_search(&fst_var, fullname))
      return;
    ...
    if (size > 1 || vpi_get(vpiLeftRange, item) != 0) {
      ...
      new_ident = fstWriterCreateVar(dump_file, type, FST_VD_IMPLICIT, size,
                                     buf, (fstHandle)(long)ident);
      free(buf);
    } else {
      new_ident = fstWriterCreateVar(dump_file, type, FST_VD_IMPLICIT, size,
                                     escname, (fstHandle)(long)ident);
    }
    free(escname);

    if (!ident) {
      if (nexus_id)
        set_nexus_ident(nexus_id, (const char *)(long)new_ident);
