...
static void show_this_item(struct vcd_info *info) {
  ...
  if (vpi_get(vpiType, info->item) == vpiRealVar) {
    ...
    lxt2_wr_emit_value_double(dump_file, info->sym, 0, value.value.real);

  } else {
    value.format = vpiBinStrVal;
    vpi_get_value(info->item, &value);
    lxt2_wr_emit_value_bit_string(dump_file, info->sym, 0 /* array row */,
                                  value.value.str);
  }
}
...
static void scan_item(unsigned depth, vpiHandle item, int skip) {
  ...
  static int types[] = {/* Value */
                        vpiNet, vpiReg, vpiVariables,
                        /* Scope */
                        vpiFunction, vpiModule, vpiNamedBegin, vpiNamedFork,
                        vpiTask, -1};
  ...
  case vpiNet:
    ...
    if (skip || vpi_get(vpiAutomatic, item))
      break;
    ...
    if (!ident) {
      ...
      lxt2_wr_symbol_alias(dump_file, ident, n, vpi_get(vpiSize, item) - 1, 0);
      free(n);
    }
  ...
  case vpiRealVar:
    ...
    info->item      = item;
    info->sym =
        lxt2_wr_symbol_add(dump_file, ident, 0 /* array rows */,
                           vpi_get(vpiSize, item) - 1, 0, LXT2_WR_SYM_F_DOUBLE);
    info->scheduled = 0;
