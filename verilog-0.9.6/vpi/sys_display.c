...
static unsigned int get_format_char(char **rtn, int ljust, int plus,
  ...
  assert(width >= -1);
  if ((unsigned int)(width + 1) > ini_size)
    ini_size = width + 1;
  ...
  case '%':
  case '\0':
    if (ljust != 0 || plus != 0 || ld_zero != 0 || width != -1 || prec != -1) {
    ...
    if (*idx >= info->nitems) {
      ...
      if (value.format == vpiSuppressVal) {
        ...
        for (word = 0; word < veclen; word += 1) {
          bits = value.value.vector[word].aval & ~value.value.vector[word].bval;
...
static char *get_display(unsigned int *rtnsz,
  ...
  for (idx = 0; idx < info->nitems; idx += 1) {
    ...
    case vpiTimeVar:
      ...
      width = strlen(buf);
      if (width < timeformat_info.width)
        width = timeformat_info.width;
      rtn = realloc(rtn, (size + width) * sizeof(char));
...
static PLI_INT32 sys_monitor_compiletf(PLI_BYTE8 *name) {
  ...
  if (sys_check_args(callh, argv, name, 1, 1))
    vpi_control(vpiFinish, 1);
  return 0;
}
