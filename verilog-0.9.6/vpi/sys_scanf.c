...
static int scan_format_base(vpiHandle callh, vpiHandle argv,
                            struct byte_source *src, unsigned width,
                            unsigned suppress_flag, PLI_BYTE8 *name,
                            const char *match, char code, PLI_INT32 type) {
  vpiHandle arg;
  char *strval = malloc(1);
  unsigned len = 0;
  s_vpi_value val;
...
static int scan_format(vpiHandle callh, struct byte_source *src, vpiHandle argv,
  ...
  while (fmtp && *fmtp != 0 && match) {
    ...
    if (isspace((int)*fmtp)) {
      ...
      case 'c':
        match =
            scan_format_char(callh, argv, src, max_width, suppress_flag, name);
        if (match == 1)
          rc += 1;
        break;

      case 'd':
        match = scan_format_decimal(callh, argv, src, max_width, suppress_flag,
                                    name);
        if (match == 1)
          rc += 1;
        break;
      ...
      case 'o':
        match =
            scan_format_octal(callh, argv, src, max_width, suppress_flag, name);
        if (match == 1)
          rc += 1;
        break;
...
static int is_assignable_obj(vpiHandle obj) {
  ...
  case vpiMemoryWord:
    ...
    break;
  case vpiPartSelect:
    if (!is_assignable_obj(vpi_handle(vpiParent, obj)))
      break;
  case vpiIntegerVar:
  case vpiRealVar:
  case vpiReg:
...
static PLI_INT32 sys_sscanf_compiletf(PLI_BYTE8 *name) {
  ...
  case vpiConstant:
  case vpiParameter:
    if (vpi_get(vpiConstType, reg) == vpiStringConst)
      break;
