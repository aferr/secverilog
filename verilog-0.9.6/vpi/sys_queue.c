...
static unsigned is_queue_full(int64_t idx) {
  if (base[idx].elems >= base[idx].length)
    return 1;
...
static void get_four_state(vpiHandle arg, p_vpi_vecval vec) {
   ...
   * extend the value. */
  if (vpi_get(vpiSigned, arg) && (size < 32)) {
    if (vec->aval & (1 << (size - 1)))
      vec->aval |= ~mask;
    if (vec->bval & (1 << (size - 1)))
      vec->bval |= ~mask;
  }
}
...
static PLI_INT32 sys_q_initialize_compiletf(PLI_BYTE8 *name) {
   ...
   * length) are numeric. */
  if (check_numeric_args(argv, 3, callh, name))
    return 0;
...
static PLI_INT32 sys_q_exam_compiletf(PLI_BYTE8 *name) {
  ...
  if (check_numeric_args(argv, 2, callh, name))
    return 0;
