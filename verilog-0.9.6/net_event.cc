...
static bool events_are_similar(NetEvent *a, NetEvent *b) {
  ...
  for (unsigned idx = 0; idx < amatched.size(); idx += 1)
    if (amatched[idx] == false)
      return false;

  for (unsigned idx = 0; idx < bmatched.size(); idx += 1)
    if (bmatched[idx] == false)
      return false;
...
NetEvWait::NetEvWait(NetProc *pr) : statement_(pr), nevents_(0), events_(0) {}
...
unsigned NetEvWait::nevents() const { return nevents_; }
