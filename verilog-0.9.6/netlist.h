...
public:
  ...
  explicit NetNet(NetScope *s, perm_string n, Type t, long ms, long ls, long s0,
                  long e0);
...
public:
  ...
  NetExpr *set_parameter(perm_string name, NetExpr *val,
                         ivl_variable_type_t type, NetExpr *msb, NetExpr *lsb,
                         bool signed_flag, NetScope::range_t *range_list,
                         const LineInfo &file_line);
  NetExpr *set_localparam(perm_string name, NetExpr *val,
                          const LineInfo &file_line);

  const NetExpr *get_parameter(const char *name, const NetExpr *&msb,
...
public:
  ...
  struct param_expr_t : public LineInfo {
    param_expr_t()
        : type(IVL_VT_NO_TYPE), signed_flag(false), msb(0), lsb(0), range(0),
          expr(0) {}
    // Type information
...
public:
  ...
  virtual bool synth_async(Design *des, NetScope *scope, const NetBus &nex_map,
                           NetBus &nex_out);
...
public:
  ...
  bool synth_async(Design *des, NetScope *scope, const NetBus &nex_map,
                   NetBus &nex_out);
