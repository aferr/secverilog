...
bool PExpr::elaborate_sig(Design *des, NetScope *scope) const { return true; }
...
bool PGate::elaborate_sig(Design *des, NetScope *scope) const { return true; }
...
bool PGModule::elaborate_sig_udp_(Design *des, NetScope *scope,
                                  PUdp *udp) const {
  bool flag = true;
...
void PFunction::elaborate_sig(Design *des, NetScope *scope) const {
  ...
  if (ports_ == 0) {
    cerr << get_fileline() << ": error: Function " << fname << " has no ports."
         << endl;
  ...
  case PTF_REG_S:
    if (return_type_.range) {
      ...
      NetExpr *me = elab_and_eval(des, scope, (*return_type_.range)[0], -1);
      assert(me);
      NetExpr *le = elab_and_eval(des, scope, (*return_type_.range)[1], -1);
      assert(le);
  ...
  default:
    if (ports_) {
      cerr << get_fileline() << ": internal error: I don't know "
           << "how to deal with return type of function " << scope->basename()
           << "." << endl;
    } else {
    ...
    for (unsigned idx = 0; idx < ports_->count(); idx += 1) {
      ...
      if (tmp == 0) {
        cerr << get_fileline() << ": internal error: function "
             << scope_path(scope) << " is missing port " << pname << "."
             << endl;
        scope->dump(cerr);
      ...
      if (tmp->port_type() != NetNet::PINPUT) {
        cerr << tmp->get_fileline() << ": error: function " << scope_path(scope)
             << " port " << pname << " is not an input port." << endl;
  ...
  NetFuncDef *def = 0;
  if (ret_sig)
    def = new NetFuncDef(scope, ret_sig, ports);

  assert(def);
  if (debug_elaborate)
    cerr << get_fileline() << ": debug: "
         << "Attach function definition to scope " << scope_path(scope) << "."
         << endl;
...
void PTask::elaborate_sig(Design *des, NetScope *scope) const {
  ...
  for (unsigned idx = 0; idx < ports.count(); idx += 1) {
    ...
    if (tmp == 0) {
      cerr << get_fileline() << ": internal error: "
           << "Could not find port " << port_name << " in scope "
           << scope_path(scope) << endl;
      scope->dump(cerr);
      des->errors += 1;
    }
...
NetNet *PWire::elaborate_sig(Design *des, NetScope *scope) const {
  ...
  if (port_set_ || net_set_) {
    ...
    if (port_set_ && port_msb_ != 0) {
      ...
      if (!eval_as_long(pmsb, texpr)) {
        cerr << port_msb_->get_fileline()
             << ": error: "
                "Range expressions must be constant."
             << endl;
        cerr << port_msb_->get_fileline()
             << "       : "
    ...
    if (!port_set_)
      assert(port_msb_ == 0 && port_lsb_ == 0);
    if (port_msb_ == 0)
      assert(port_lsb_ == 0);
    if (port_lsb_ == 0)
      assert(port_msb_ == 0);
    ...
    if (port_set_ && net_set_ && (pmsb != nmsb || plsb != nlsb)) {
      ...
      if (port_msb_ == 0) {
        if (!gn_io_range_error_flag) {
          cerr << get_fileline() << ": warning: Scalar port ``" << name_
               << "'' has a vectored net declaration [" << nmsb << ":" << nlsb
               << "]." << endl;
        } else {
      ...
      if (net_msb_ == 0) {
        cerr << port_msb_->get_fileline() << ": error: Vectored port ``"
             << name_ << "'' [" << pmsb << ":" << plsb
      ...
      if (port_msb_ != 0 && net_msb_ != 0) {
        cerr << port_msb_->get_fileline() << ": error: Vectored port ``"
             << name_ << "'' [" << pmsb << ":" << plsb
             << "] has a net declaration [" << nmsb << ":" << nlsb << "] at "
             << net_msb_->get_fileline() << " that does not match." << endl;
        des->errors += 1;
        return 0;
      }
    }
  ...
  if (lidx_ || ridx_) {
    ...
    if ((lexp == 0) || (rexp == 0)) {
      cerr << get_fileline() << ": internal error: There is "
           << "a problem evaluating indices for ``" << name_ << "''." << endl;
      des->errors += 1;
      return 0;
    }
    ...
    if (!const_flag) {
      cerr << get_fileline() << ": error: The indices "
           << "are not constant for array ``" << name_ << "''." << endl;
      des->errors += 1;
