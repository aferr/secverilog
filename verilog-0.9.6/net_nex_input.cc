...
NexusSet *NetEAccess::nex_input(bool rem_out) { return new NexusSet; }

/*
 * A constant has not inputs, so always return an empty set.
 */
NexusSet *NetEConst::nex_input(bool rem_out) { return new NexusSet; }

NexusSet *NetECReal::nex_input(bool rem_out) { return new NexusSet; }

/*
 * A parameter by definition has no inputs. It represents a constant
 * value, even if that value is a constant expression.
 */
NexusSet *NetEParam::nex_input(bool rem_out) { return new NexusSet; }

NexusSet *NetEEvent::nex_input(bool rem_out) { return new NexusSet; }

NexusSet *NetEScope::nex_input(bool rem_out) { return new NexusSet; }
...
NexusSet *NetESFunc::nex_input(bool rem_out) {
  ...
  NexusSet *result;
  if (parms_[0])
    result = parms_[0]->nex_input(rem_out);
  else
    result = new NexusSet;
  ...
  if (word_) {
    ...
    if (warn_sens_entire_arr) {
      cerr << get_fileline() << ": warning: @* is sensitive to all "
           << net_->array_count() << " words in array '" << name() << "'."
           << endl;
    }
  }
 ...
 */
NexusSet *NetUTask::nex_input(bool rem_out) { return new NexusSet; }
