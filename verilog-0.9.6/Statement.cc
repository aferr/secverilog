...
void PAssign::mustmodify(set<PExpr*, ExprComparator>& vars, set<perm_string> depexprs) const
{
...
void PAssignNB::mustmodify(set<PExpr*, ExprComparator>& vars, set<perm_string> depexprs) const
{
	if (depexprs.find(lval_->get_name())!=depexprs.end())
		vars.insert(lval_);
}
...
void PBlock::mustmodify(set<PExpr*, ExprComparator>& vars, set<perm_string> depexprs) const
{
	for (unsigned idx = 0; idx < list_.count(); idx += 1) {
		if (list_[idx])
...
void PCondit::mustmodify(set<PExpr*, ExprComparator>& vars, set<perm_string> depexprs) const
{
	...
	std::set_intersection( if_modified.begin(), if_modified.end(), else_modified.begin(), else_modified.end(),
