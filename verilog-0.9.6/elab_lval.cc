 ...
 */
NetAssign_ *PExpr::elaborate_lval(Design *des, NetScope *scope,
                                  bool is_force) const {
  NetNet *ll = 0;
  if (ll == 0) {
...
NetAssign_ *PEIdent::elaborate_lval(Design *des, NetScope *scope,
  ...
  if ((reg->type() != NetNet::REG) && !is_force) {
    cerr << get_fileline() << ": error: " << path_
         << " is not a valid l-value in " << scope_path(scope) << "." << endl;
...
NetAssign_ *PEIdent::elaborate_lval_net_word_(Design *des, NetScope *scope,
  ...
  if (long base = reg->array_first()) {
    ...
    word = normalize_variable_array_base(word, base, reg->array_count());
    eval_expr(word);
  }
  ...
  if (NetEConst *word_const = dynamic_cast<NetEConst *>(word)) {
    ...
    if (index < 0 || index >= (long)reg->array_count()) {
      cerr << get_fileline() << ": warning: Constant array index "
           << (index + reg->array_first()) << " is out of range for array "
           << reg->name() << "." << endl;
    }
  }
  ...
  if (reg->get_scalar() && use_sel != index_component_t::SEL_NONE) {
    ...
    if (reg->data_type() == IVL_VT_REAL)
      ...
      cerr << "scalar";
    cerr << " array word: " << reg->name() << "[" << *word << "]" << endl;
    des->errors += 1;
    return 0;
  }
...
bool PEIdent::elaborate_lval_net_bit_(Design *des, NetScope *scope,
  ...
  if (mux) {
    ...
    if (loff < 0 || loff >= (long)reg->vector_width()) {
      cerr << get_fileline() << ": error: bit select " << reg->name() << "["
           << lsb << "]"
...
bool PEIdent::elaborate_lval_net_idx_(
  ...
  if (NetEConst *base_c = dynamic_cast<NetEConst *>(base)) {
    ...
    if (base_c->value().is_defined()) {
      ...
      if (rel_base == 0 && wid == reg->vector_width())
        return true;
    ...
    if (use_sel == index_component_t::SEL_IDX_UP) {
      base = normalize_variable_base(base, reg->msb(), reg->lsb(), wid, true);
    } else {
      // This is assumed to be a SEL_IDX_DO.
      base = normalize_variable_base(base, reg->msb(), reg->lsb(), wid, false);
    }
  }
