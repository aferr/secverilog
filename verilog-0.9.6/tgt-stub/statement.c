...
static unsigned show_assign_lval(ivl_lval_t lval, unsigned ind) {
  ...
  fprintf(out, "%*s{name=%s width=%u lvwidth=%u}\n", ind, "",
          ivl_signal_name(sig), ivl_signal_width(sig), ivl_lval_width(lval));
  ...
  if (ivl_lval_idx(lval)) {
    ...
    if (ivl_signal_dimensions(sig) < 1) {
      fprintf(out,
              "%*sERROR: Address on signal with "
              "array dimensions=%u\n",
              ind + 4, "", ivl_signal_dimensions(sig));
      stub_errors += 1;
    }
  } else if (ivl_signal_array_count(sig) > 1) {
    fprintf(out,
            "%*sERROR: Address missing on signal with "
            "word count=%u\n",
            ind + 4, "", ivl_signal_array_count(sig));
    stub_errors += 1;
  }
...
void show_stmt_wait(ivl_statement_t net, unsigned ind) {
  ...
  for (idx = 0; idx < ivl_stmt_nevent(net); idx += 1) {
    ...
    if (evnt == 0)
      ...
      fprintf(out, "%s%s.%s", comma, ivl_scope_name(ivl_event_scope(evnt)),
...
void show_statement(ivl_statement_t net, unsigned ind) {
  ...
  case IVL_ST_STASK: {
    fprintf(out, "%*sCall %s(%u parameters); /* %s:%u */\n", ind, "",
            ivl_stmt_name(net), ivl_stmt_parm_count(net), ivl_stmt_file(net),
            ivl_stmt_lineno(net));
