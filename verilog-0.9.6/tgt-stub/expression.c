...
static void show_array_expression(ivl_expr_t net, unsigned ind) {
  ...
  fprintf(out,
          "%*sArray: %s, word_count=%u (%u dimensions), width=%u, type=%s\n",
  ...
  case '*':
    if (ivl_expr_value(net) == IVL_VT_REAL) {
      ...
      if (ivl_expr_width(net) > max_width) {
        fprintf(out,
                "%*sERROR: Result width to width. Expecting <= %u, got %u\n",
...
void show_expression(ivl_expr_t net, unsigned ind) {
  ...
  case IVL_EX_CONCAT:
    fprintf(out, "%*s<concat repeat=%u, width=%u, %s, type=%s>\n", ind, "",
            ivl_expr_repeat(net), width, sign, vt);
    for (idx = 0; idx < ivl_expr_parms(net); idx += 1)
  ...
  case IVL_EX_NUMBER: {
    ...
    if (par != 0)
      fprintf(out, ", parameter=%s", ivl_parameter_basename(par));
  ...
  case IVL_EX_STRING:
    fprintf(out, "%*s<string=\"%s\", width=%u", ind, "", ivl_expr_string(net),
            ivl_expr_width(net));
    if (par != 0)
      fprintf(out, ", parameter=%s", ivl_parameter_basename(par));
  ...
  case IVL_EX_SFUNC:
    fprintf(out, "%*s<function=\"%s\", width=%u, %s, type=%s file=%s:%u>\n",
            ind, "", ivl_expr_name(net), width, sign, vt, ivl_expr_file(net),
            ivl_expr_lineno(net));
    {
