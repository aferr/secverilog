...
void AContrib::absintp(Predicate &pred) const { throw "AContrib"; }

/**
 * For blocking assignment, clear predicates on the lhs. Set up a new predicate
 * on lhs if the rhs is a number.
 */
void PAssign::absintp(Predicate &pred, TypeEnv &env) const {
  ...
  if (number != NULL ||
      env.dep_exprs.find(rval_->get_name()) != env.dep_exprs.end()) {
 ...
 */
void PCondit::absintp(Predicate &pred, TypeEnv &env, bool istrue,
                      bool useAllExprs) const {
  ...
  if (istrue && if_) {
    if (useAllExprs || expr_->is_wellformed(env.dep_exprs)) {
      if (condvar != NULL) {
        pred.hypotheses.insert(new Hypothesis(
            (useAllExprs) ? expr_ : expr_->to_wellformed(env.dep_exprs),
    ...
    if (useAllExprs || expr_->is_neg_wellformed(env.dep_exprs)) {
      if (condvar != NULL) {
        pred.hypotheses.insert(new Hypothesis(
            (useAllExprs) ? expr_ : expr_->neg_to_wellformed(env.dep_exprs),
            new PENumber(new verinum((uint64_t)0, 32))));
      } else {
        pred.hypotheses.insert(new Hypothesis(new PEUnary(
            'N',
            (useAllExprs) ? expr_ : expr_->neg_to_wellformed(env.dep_exprs))));
      }
    }
...
void PForce::absintp(Predicate &pred, TypeEnv &env) const { throw "PForce"; }
...
void PRelease::absintp(Predicate &pred, TypeEnv &) const { throw "PRelease"; }

void PRepeat::absintp(Predicate &pred, TypeEnv &) const { throw "PRepeat"; }

void PTrigger::absintp(Predicate &pred, TypeEnv &) const { throw "PTrigger"; }

void PWhile::absintp(Predicate &pred, TypeEnv &) const { throw "PWhile"; }
