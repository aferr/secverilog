...
static void collect_scope_parameters_(
  ...
  for (mparm_it_t cur = parameters.begin(); cur != parameters.end(); cur++) {
    ...
    scope->set_parameter((*cur).first, tmp, (*cur).second.type, 0, 0, false, 0,
                         (*cur).second);
  }
}
...
static void elaborate_scope_parameters_(
    Design *des, NetScope *scope,
    const map<perm_string, LexicalScope::param_expr_t> &parameters) {
  for (mparm_it_t cur = parameters.begin(); cur != parameters.end(); cur++) {
    ...
    if (scope->find_genvar((*cur).first)) {
      cerr << cur->second.get_fileline() << ": error: parameter and genvar in '"
           << scope->fullname() << "' have the same name '" << (*cur).first
           << "'." << endl;
      des->errors += 1;
    }
...
static void elaborate_scope_localparams_(
    Design *des, NetScope *scope,
    const map<perm_string, LexicalScope::param_expr_t> &localparams) {
  for (mparm_it_t cur = localparams.begin(); cur != localparams.end(); cur++) {
    ...
    if (scope->find_genvar((*cur).first)) {
      cerr << cur->second.get_fileline()
           << ": error: localparam and genvar in '" << scope->fullname()
           << "' have the same name '" << (*cur).first << "'." << endl;
      des->errors += 1;
    }
...
static void elaborate_scope_tasks(Design *des, NetScope *scope,
  ...
  for (tasks_it_t cur = tasks.begin(); cur != tasks.end(); cur++) {
    ...
    if (child) {
      ...
      cerr << " in '" << scope->fullname() << "' have the same name '"
           << use_name << "'." << endl;
      des->errors += 1;
      continue;
    }
    ...
    if (scope->find_genvar((*cur).first)) {
      cerr << cur->second->get_fileline() << ": error: task and genvar in '"
           << scope->fullname() << "' have the same name '" << (*cur).first
           << "'." << endl;
      des->errors += 1;
    }
    ...
    const NetExpr *parm = scope->get_parameter((*cur).first, ex_msb, ex_lsb);
    if (parm) {
      cerr << cur->second->get_fileline() << ": error: task and parameter in '"
           << scope->fullname() << "' have the same name '" << (*cur).first
           << "'." << endl;
      des->errors += 1;
    }
...
static void elaborate_scope_funcs(Design *des, NetScope *scope,
  ...
  for (funcs_it_t cur = funcs.begin(); cur != funcs.end(); cur++) {
    ...
    const NetScope *child = scope->child(use_name);
    if (child) {
      cerr << cur->second->get_fileline() << ": error: function and ";
      child->print_type(cerr);
      cerr << " in '" << scope->fullname() << "' have the same name '"
           << use_name << "'." << endl;
      des->errors += 1;
    ...
    if (scope->find_genvar((*cur).first)) {
      cerr << cur->second->get_fileline() << ": error: function and genvar in '"
           << scope->fullname() << "' have the same name '" << (*cur).first
           << "'." << endl;
      des->errors += 1;
    }
    ...
    if (parm) {
      cerr << cur->second->get_fileline()
           << ": error: function and parameter in '" << scope->fullname()
           << "' have the same name '" << (*cur).first << "'." << endl;
      des->errors += 1;
    }

    NetScope *func_scope = new NetScope(scope, use_name, NetScope::FUNC);
...
bool Module::elaborate_scope(Design *des, NetScope *scope,
  ...
  if (debug_scopes) {
    cerr << get_fileline() << ": debug: Elaborate scope " << scope_path(scope)
         << "." << endl;
  }
...
bool PGenerate::generate_scope_loop_(Design *des, NetScope *container) {
  ...
  if (child) {
    ...
    child->print_type(cerr);
    cerr << " in '" << container->fullname() << "' have the same name '"
         << scope_name << "'." << endl;
    des->errors += 1;
    return false;
  }
  ...
  if (container->find_genvar(scope_name)) {
    cerr << get_fileline()
         << ": error: generate \"loop\" and "
            "genvar in '"
         << container->fullname() << "' have the same name '" << scope_name
         << "'." << endl;
    des->errors += 1;
  }
  ...
  if (container->get_parameter(loop_index, tmsb, tlsb))
    return false;
  if (container->find_event(loop_index))
    return false;
...
bool PGenerate::generate_scope_condit_(Design *des, NetScope *container,
  ...
  if (event) {
    cerr << get_fileline()
         << ": error: generate \"if\" and "
            "named event in '"
         << container->fullname() << "' have the same name '" << use_name
         << "'." << endl;
    des->errors += 1;
  }
  ...
  const NetExpr *ex_msb, *ex_lsb;
  const NetExpr *parm = container->get_parameter(scope_name, ex_msb, ex_lsb);
  if (parm) {
    cerr << get_fileline()
         << ": error: generate \"if\" and "
            "parameter in '"
...
bool PGenerate::generate_scope_case_(Design *des, NetScope *container) {
  ...
  while (cur != generate_schemes.end()) {
    ...
    for (unsigned idx = 0; idx < item->item_test.size() && !match_flag;
      ...
      NetExpr *item_value_ex =
          elab_and_eval(des, container, item->item_test[idx], -1);
      need_constant_expr       = false;
  ...
  const NetExpr *ex_msb, *ex_lsb;
  const NetExpr *parm =
      container->get_parameter(item->scope_name, ex_msb, ex_lsb);
  if (parm) {
    cerr << get_fileline()
         << ": error: generate \"case\" and "
            "parameter in '"
...
void PGModule::elaborate_scope_mod_(Design *des, Module *mod,
  ...
  if (child) {
    cerr << get_fileline() << ": error: module <" << mod->mod_name()
         << "> instance and ";
    child->print_type(cerr);
    cerr << " in '" << sc->fullname() << "' have the same name '" << get_name()
         << "'." << endl;
    des->errors += 1;
    return;
  }
...
void PGModule::elaborate_scope_mod_instances_(Design *des, Module *mod,
                                              NetScope *sc) const {
  if (msb_)
    probe_expr_width(des, sc, msb_);
  if (lsb_)
    probe_expr_width(des, sc, lsb_);
  need_constant_expr = true;
  ...
  for (int idx = 0; idx < instance_count; idx += 1) {
    ...
    for (mparm_itr_t cur = replace.begin(); cur != replace.end(); cur++) {
...
void PEvent::elaborate_scope(Design *des, NetScope *scope) const {
  ...
  if (child) {
    ...
    child->print_type(cerr);
    cerr << " in '" << scope->fullname() << "' have the same name '" << name_
         << "'." << endl;
    des->errors += 1;
  }
  ...
  if (scope->find_genvar(name_)) {
    cerr << get_fileline() << ": error: named event and "
         << "genvar in '" << scope->fullname() << "' have the same name '"
         << name_ << "'." << endl;
    des->errors += 1;
  }
  ...
  if (parm) {
    cerr << get_fileline() << ": error: named event and "
         << "parameter in '" << scope->fullname() << "' have the same name '"
         << name_ << "'." << endl;
    des->errors += 1;
  }
...
void PBlock::elaborate_scope(Design *des, NetScope *scope) const {
  ...
  if (pscope_name() != 0) {
    ...
    if (scope->find_genvar(pscope_name())) {
      cerr << get_fileline()
           << ": error: named block and "
              "genvar in '"
           << scope->fullname() << "' have the same name '" << use_name << "'."
           << endl;
      des->errors += 1;
    }
    ...
    const NetExpr *ex_msb, *ex_lsb;
    const NetExpr *parm = scope->get_parameter(pscope_name(), ex_msb, ex_lsb);
    if (parm) {
      cerr << get_fileline()
           << ": error: named block and "
              "parameter in '"
    ...
    my_scope = new NetScope(scope, use_name,
                            bl_type_ == BL_PAR ? NetScope::FORK_JOIN
                                               : NetScope::BEGIN_END);
