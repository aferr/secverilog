...
int acc_object_of_type(handle object, PLI_INT32 type) {
  ...
  if (pli_trace) {
    fprintf(pli_trace, "acc_object_of_type(%p \"%s\", %d)", (void *)object,
  ...
  case accScope:
    if (vtype == vpiModule || vtype == vpiNamedBegin || vtype == vpiNamedFork ||
        vtype == vpiTask || vtype == vpiFunction)
      rtn = 1;
    break;
  ...
  case accRealParam:
    if (vtype == vpiNamedEvent && vpi_get(vpiConstType, object) == vpiRealConst)
      rtn = 1;
    break;
