...
static perm_string make_scope_name(const hname_t &name) {
  ...
  char buf[1024];
  snprintf(buf, sizeof buf, "%s[%d]", name.peek_name().str(),
           name.peek_number());
...
void dll_target::add_root(ivl_design_s &des__, const NetScope *s) {
  ...
  if (des__.roots_)
    des__.roots_ = (ivl_scope_t *)realloc(des__.roots_,
                                          des__.nroots_ * sizeof(ivl_scope_t));
  else
...
void dll_target::lpm_mux(const NetMux *net) {
  ...
  nexus_lpm_add(obj->u_.mux.q, obj, 0, IVL_DR_STRONG, IVL_DR_STRONG);
...
void dll_target::signal(const NetNet *net) {
  ...
  if (obj->scope_->nsigs_ == 0) {
    ...
    obj->scope_->nsigs_ += 1;
    obj->scope_->sigs_ = (ivl_signal_t *)realloc(
        obj->scope_->sigs_, obj->scope_->nsigs_ * sizeof(ivl_signal_t));
  }
