...
NetExpr *PExpr::elaborate_pexpr(Design *des, NetScope *sc) const {
  cerr << get_fileline() << ": error: invalid parameter expression: " << *this
       << endl;
  des->errors += 1;
  ...
  for (unsigned idx = 0; idx < parms_.count(); idx += 1) {
    ...
    if (dynamic_cast<NetEParam *>(ex)) {
      ...
      cerr << ex->get_fileline() << ": error: operand of "
           << "concatenation has indefinite width: " << *ex << endl;
      des->errors += 1;
      fail = true;
      continue;
    }
...
NetExpr *PEIdent::elaborate_pexpr(Design *des, NetScope *scope) const {
  ...
  if (path_.size() > 1) {
    cerr << get_fileline()
         << ": error: parameter r-value expression "
            "does not support hierarchical references `"
         << path_ << "`." << endl;
    des->errors += 1;
    return 0;
  }
...
NetExpr *PECallFunction::elaborate_pexpr(Design *des, NetScope *scope) const {
  ...
  if (nm[0] == '$' && (generation_flag >= GN_VER2005 || gn_icarus_misc_flag ||
                       gn_verilog_ams_flag)) {
    if (nm == "$clog2" || nm == "$ln" || nm == "$log10" || nm == "$exp" ||
        nm == "$sqrt" || nm == "$floor" || nm == "$ceil" || nm == "$sin" ||
        nm == "$cos" || nm == "$tan" || nm == "$asin" || nm == "$acos" ||
        nm == "$atan" || nm == "$sinh" || nm == "$cosh" || nm == "$tanh" ||
        nm == "$asinh" || nm == "$acosh" || nm == "$atanh") {
    ...
    if ((gn_icarus_misc_flag || gn_verilog_ams_flag) && (nm == "$abs")) {
      if (parms_.size() != 1 || parms_[0] == 0) {
        cerr << get_fileline() << ": error: " << nm
             << " takes a single argument." << endl;
        des->errors += 1;
  ...
  cerr << get_fileline()
       << ": sorry: constant user functions are not "
          "currently supported: "
       << nm << "()." << endl;
  des->errors += 1;
  return 0;
