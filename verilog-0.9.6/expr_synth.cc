...
static bool process_binary_args(Design *des, NetScope *scope, NetExpr *root,
                                ...
                                NetNet *&rsig, bool &real_args, NetExpr *obj) {
...
NetNet *NetEBBits::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  assert(lsig->vector_width() == rsig->vector_width());
  NetNet *osig =
      new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, width);
  osig->local_flag(true);
  osig->data_type(expr_type());
...
NetNet *NetEBComp::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  NetNet *osig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, 1);
  osig->set_line(*this);
  osig->local_flag(true);
  ...
  if (op_ == 'E' || op_ == 'N') {
    NetCaseCmp *gate = new NetCaseCmp(scope, scope->local_symbol(), width,
                                      op_ == 'E' ? true : false);
    gate->set_line(*this);
  ...
  if ((width == 1) && (op_ == 'e') && !real_args) {
    NetLogic *gate =
        new NetLogic(scope, scope->local_symbol(), 3, NetLogic::XNOR, 1);
    gate->set_line(*this);
    connect(gate->pin(0), osig->pin(0));
    connect(gate->pin(1), lsig->pin(0));
    connect(gate->pin(2), rsig->pin(0));
  ...
  if ((width == 1) && (op_ == 'n') && !real_args) {
    NetLogic *gate =
        new NetLogic(scope, scope->local_symbol(), 3, NetLogic::XOR, 1);
    gate->set_line(*this);
    connect(gate->pin(0), osig->pin(0));
    connect(gate->pin(1), lsig->pin(0));
    connect(gate->pin(2), rsig->pin(0));
...
NetNet *NetEBDiv::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  case '/': {
    NetDivide *div = new NetDivide(scope, scope->local_symbol(), width,
                                   lsig->vector_width(), rsig->vector_width());
    div->set_line(*this);
  ...
  case '%': {
    ...
    NetModulo *div = new NetModulo(scope, scope->local_symbol(), width,
                                   lsig->vector_width(), rsig->vector_width());
    div->set_line(*this);
    div->set_signed(has_sign());
    des->add_node(div);
...
NetNet *NetEBLogic::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  if (op() == 'o') {
    ...
    NetLogic *olog =
        new NetLogic(scope, oname, lsig->pin_count() + rsig->pin_count() + 1,
                     NetLogic::OR, 1);
...
NetNet *NetEBShift::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  if (NetEConst *rcon = dynamic_cast<NetEConst *>(right_)) {
    ...
    NetNet *osig = new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT,
                              expr_width());
    osig->data_type(expr_type());
    osig->local_flag(true);
    ...
    NetPartSelect *psel = new NetPartSelect(lsig, shift < 0 ? ushift : 0,
                                            part_width, NetPartSelect::VP);
    des->add_node(psel);

    NetNet *psig =
        new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, part_width);
    ...
    NetNet *zsig =
        new NetNet(scope, scope->local_symbol(), NetNet::WIRE, znum.len());
  ...
  NetNet *osig =
      new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, expr_width());
  osig->data_type(expr_type());
  osig->local_flag(true);
...
NetNet *NetESelect::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  if (NetEConst *base_const = dynamic_cast<NetEConst *>(base_)) {
    ...
    NetNet *tmp =
        new NetNet(scope, scope->local_symbol(), NetNet::WIRE, select_width);
    ...
    if (concat_count > 1) {
      ...
      tmp =
          new NetNet(scope, scope->local_symbol(), NetNet::WIRE, expr_width());
      tmp->data_type(sub->data_type());
      tmp->local_flag(true);
      tmp->set_line(*this);
  ...
  if (has_sign()) {
    ...
    NetNet *tmp =
        new NetNet(scope, scope->local_symbol(), NetNet::IMPLICIT, pad_width);
...
NetNet *NetESFunc::synthesize(Design *des, NetScope *scope, NetExpr *root) {
  ...
  NetSysFunc *net =
      new NetSysFunc(scope, scope->local_symbol(), def, 1 + nparms_, trigger);
  net->set_line(*this);
  des->add_node(net);

  NetNet *osig =
      new NetNet(scope, scope->local_symbol(), NetNet::WIRE, def->wid);
  osig->local_flag(true);
