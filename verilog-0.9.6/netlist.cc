...
NetPins::NetPins(unsigned npins) : npins_(npins) {
  ...
  if (disable_virtual_pins)
    devirtualize_pins(); // Ask.  Bummer.
}
...
void NetPins::set_default_init(verinum::V val) { default_init_ = val; }
...
NetNet::NetNet(NetScope *s, perm_string n, Type t, unsigned npins)
    : NetObj(s, n, 1), type_(t), port_type_(NOT_A_PORT),
      data_type_(IVL_VT_NO_TYPE), signed_(false), isint_(false),
      is_scalar_(false), discipline_(0), msb_(npins - 1), lsb_(0),
      dimensions_(0), s0_(0), e0_(0), local_flag_(false), eref_count_(0),
      lref_count_(0) {
  assert(s);
  assert(npins > 0);
...
void NetNet::initialize_value_and_dir(verinum::V init_value, Link::DIR dir) {
  if (pins_are_virtual()) {
    if (0)
      cerr << "NetNet setting Link default value and dir" << endl;
...
NetNet::NetNet(NetScope *s, perm_string n, Type t, long ms, long ls)
    : NetObj(s, n, 1), type_(t), port_type_(NOT_A_PORT),
      data_type_(IVL_VT_NO_TYPE), signed_(false), isint_(false),
      is_scalar_(false), discipline_(0), msb_(ms), lsb_(ls), dimensions_(0),
...
NetNet::NetNet(NetScope *s, perm_string n, Type t, long ms, long ls,
               long array_s, long array_e)
    : NetObj(s, n, calculate_count(array_s, array_e)), type_(t),
      port_type_(NOT_A_PORT), data_type_(IVL_VT_NO_TYPE), signed_(false),
      isint_(false), is_scalar_(false), discipline_(0), msb_(ms), lsb_(ls),
      dimensions_(1), s0_(array_s), e0_(array_e), local_flag_(false),
...
ivl_variable_type_t NetNet::data_type() const { return data_type_; }

void NetNet::data_type(ivl_variable_type_t t) { data_type_ = t; }
...
void NetNet::set_signed(bool flag) { signed_ = flag; }

bool NetNet::get_isint() const { return isint_; }

void NetNet::set_isint(bool flag) { isint_ = flag; }

bool NetNet::get_scalar() const { return is_scalar_; }

void NetNet::set_scalar(bool flag) { is_scalar_ = flag; }
...
long NetNet::lsb() const { return lsb_; }

long NetNet::msb() const { return msb_; }
...
unsigned NetNet::array_dimensions() const { return dimensions_; }
...
unsigned NetNet::peek_lref() const { return lref_count_; }
...
NetPartSelect::NetPartSelect(NetNet *sig, unsigned off, unsigned wid,
    ...
    : NetNode(sig->scope(), sig->scope()->local_symbol(), 2), off_(off),
      wid_(wid), dir_(dir__), signed_flag_(false) {
  set_line(*sig);
...
NetPartSelect::NetPartSelect(NetNet *sig, NetNet *sel, unsigned wid,
                             bool signed_flag__)
    : NetNode(sig->scope(), sig->scope()->local_symbol(), 3), off_(0),
      wid_(wid), dir_(VP), signed_flag_(signed_flag__) {
  switch (dir_) {
  case NetPartSelect::VP:
    pin(0).set_dir(Link::OUTPUT);
    pin(1).set_dir(Link::INPUT);
...
NetPartSelect::dir_t NetPartSelect::dir() const { return dir_; }
...
NetProcTop::~NetProcTop() { delete statement_; }
...
NetCLShift::NetCLShift(NetScope *s, perm_string n, unsigned width__,
                       ...
                       bool signed_flag__)
    : NetNode(s, n, 3), width_(width__), width_dist_(width_dist__),
...
NetDivide::NetDivide(NetScope *sc, perm_string n, unsigned wr, unsigned wa,
                     unsigned wb)
    : NetNode(sc, n, 3), width_r_(wr), width_a_(wa), width_b_(wb),
      signed_flag_(false) {
  pin(0).set_dir(Link::OUTPUT); // Result
  pin(1).set_dir(Link::INPUT);  // DataA
  pin(2).set_dir(Link::INPUT);  // DataB
...
NetMult::NetMult(NetScope *sc, perm_string n, unsigned wr, unsigned wa,
                 unsigned wb)
    : NetNode(sc, n, 3), signed_(false), width_r_(wr), width_a_(wa),
      width_b_(wb) {
  pin(0).set_dir(Link::OUTPUT); // Result
  pin(1).set_dir(Link::INPUT);  // DataA
  pin(2).set_dir(Link::INPUT);  // DataB
...
bool NetCaseCmp::eeq() const { return eeq_; }
...
NetConst::~NetConst() { delete[] value_; }
...
unsigned NetConst::width() const { return width_; }
...
const NetScope *NetFuncDef::scope() const { return scope_; }
...
const NetProc *NetFuncDef::proc() const { return statement_; }

NetScope *NetFuncDef::scope() { return scope_; }

unsigned NetFuncDef::port_count() const { return ports_.count(); }
...
unsigned NetSTask::nparms() const { return parms_.count(); }
...
unsigned NetEUFunc::parm_count() const { return parms_.count(); }
...
NetExpr::NetExpr(unsigned w) : width_(w), signed_flag_(false) {}

NetExpr::~NetExpr() {}

bool NetExpr::has_sign() const { return signed_flag_; }

void NetExpr::cast_signed(bool flag) { signed_flag_ = flag; }
...
NetEBBits *NetEBBits::dup_expr() const {
  NetEBBits *result = new NetEBBits(op_, left_->dup_expr(), right_->dup_expr());
  return result;
}
...
NetEBLogic *NetEBLogic::dup_expr() const {
  NetEBLogic *result =
      new NetEBLogic(op_, left_->dup_expr(), right_->dup_expr());
  return result;
}
...
long NetESignal::lsi() const { return net_->lsb(); }

long NetESignal::msi() const { return net_->msb(); }

ivl_variable_type_t NetESignal::expr_type() const { return net_->data_type(); }
...
NetEUBits::NetEUBits(char op__, NetExpr *ex) : NetEUnary(op__, ex) {}

NetEUBits::~NetEUBits() {}

ivl_variable_type_t NetEUBits::expr_type() const { return expr_->expr_type(); }

NetEUReduce::NetEUReduce(char op__, NetExpr *ex) : NetEUnary(op__, ex) {
  expr_width(1);
}
...
unsigned NetTaskDef::port_count() const { return ports_.count(); }
...
#endif
const NetScope *NetTaskDef::scope() const { return scope_; }

const NetProc *NetTaskDef::proc() const { return proc_; }
...
DelayType NetProc::delay_type() const { return NO_DELAY; }
...
DelayType NetCase::delay_type() const {
  ...
  if (!def_stmt)
    result = combine_delays(NO_DELAY, result);
...
DelayType NetEvWait::delay_type() const { return DEFINITE_DELAY; }

DelayType NetForever::delay_type() const { return statement_->delay_type(); }
...
DelayType NetTaskDef::delay_type() const { return proc_->delay_type(); }
