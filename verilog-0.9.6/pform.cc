...
PTask *pform_push_task_scope(const struct vlltype &loc, char *name,
  ...
  PTask *task;
  if (pform_cur_generate) {
    task = new PTask(task_name, pform_cur_generate->lexical_scope, is_auto);
    FILE_NAME(task, loc);
    ...
    if (pform_cur_generate->tasks.find(task->pscope_name()) !=
      ...
      cerr << task->get_fileline()
           << ": error: duplicate "
              "definition for task '"
           << name << "' in '" << pform_cur_module->mod_name()
           << "' (generate)." << endl;
      error_count += 1;
    }
    ...
    if (pform_cur_module->tasks.find(task->pscope_name()) !=
      ...
      cerr << task->get_fileline()
           << ": error: duplicate "
              "definition for task '"
           << name << "' in '" << pform_cur_module->mod_name() << "'." << endl;
      error_count += 1;
    }
...
PFunction *pform_push_function_scope(const struct vlltype &loc, char *name,
  ...
  PFunction *func;
  if (pform_cur_generate) {
    func = new PFunction(func_name, pform_cur_generate->lexical_scope, is_auto);
    FILE_NAME(func, loc);
...
static bool pform_at_module_level() {
  if (pform_cur_generate)
    if (pform_cur_generate->lexical_scope)
      ...
      return true;
  else if (lexical_scope->pscope_parent())
    return false;
  else
    return true;
}
...
void pform_set_default_nettype(NetNet::Type type, const char *file,
  ...
  if (pform_cur_module) {
    ...
    cerr << file << ":" << lineno << ":      : "
         << "module " << pform_cur_module->mod_name() << " starts on line "
...
void pform_set_timescale(int unit, int prec, const char *file,
  ...
  if (!warn_timescale || !first_flag || !file)
    return;
  ...
  for (mod = pform_modules.begin(); mod != pform_modules.end(); mod++) {
    ...
    if (mp->time_from_timescale || mp->timescale_warn_done)
      continue;
    have_no_ts = true;
    break;
  }
...
void pform_startmodule(const char *name, const char *file, unsigned lineno,
  ...
  if (warn_timescale && pform_timescale_file &&
    ...
    cerr << pform_cur_module->get_fileline() << ": warning: "
         << "timescale for " << name << " inherited from another file." << endl;
...
static void pform_add_genvar(const struct vlltype &li, const perm_string &name,
  ...
  if (genvars.find(name) != genvars.end()) {
    cerr << lni->get_fileline() << ": error: genvar '" << name
...
void pform_start_generate_for(const struct vlltype &li, char *ident1,
                              PExpr *init, PExpr *test, char *ident2,
                              PExpr *next) {
...
void pform_make_udp(perm_string name, bool synchronous_flag,
  ...
  pins[0] =
      new PWire(out_name, synchronous_flag ? NetNet::REG : NetNet::WIRE,
  ...
  /* Make the PWire objects for the input ports. */
  {
    ...
    for (cur = parms->begin(), idx = 1; cur != parms->end(); idx += 1, cur++) {
      ...
      pins[idx] = new PWire(*cur, NetNet::WIRE, NetNet::PINPUT, new ConstType(),
                            new BaseType(), IVL_VT_LOGIC);
 ...
 */
static void pform_set_net_range(perm_string name, const svector<PExpr *> *range,
                                bool signed_flag, ivl_variable_type_t dt,
                                SecType *st, BaseType *bt, PWSRType rt) {
...
void pform_set_net_range(list<perm_string> *names, svector<PExpr *> *range,
                         bool signed_flag, ivl_variable_type_t dt,
                         PWSRType rt) {
  pform_set_net_range_type(names, range, signed_flag, dt, new ConstType(),
                           new BaseType(), rt);
}

void pform_set_net_range_type(list<perm_string> *names, svector<PExpr *> *range,
                              bool signed_flag, ivl_variable_type_t dt,
                              SecType *st, BaseType *bt, PWSRType rt) {
 ...
 */
void pform_makegate(PGBuiltin::Type type, struct str_pair_t str,
                    svector<PExpr *> *delay, const lgate &info,
                    svector<named_pexpr_t *> *attr) {
...
void pform_makegates(PGBuiltin::Type type, struct str_pair_t str,
...
static void pform_make_modgate(perm_string type, perm_string name,
                               ...
                               svector<PExpr *> *wires, PExpr *msb, PExpr *lsb,
...
static void pform_make_modgate(perm_string type, perm_string name,
                               struct parmvalue_t *overrides,
                               svector<named_pexpr_t *> *bind, PExpr *msb,
...
void pform_make_modgates(perm_string type, struct parmvalue_t *overrides,
                         svector<lgate> *gates) {
  ...
  for (unsigned idx = 0; idx < gates->count(); idx += 1) {
    ...
    if (cur.parms_by_name) {
      pform_make_modgate(type, cur_name, overrides, cur.parms_by_name,
                         cur.range[0], cur.range[1], cur.file, cur.lineno);
      ...
      pform_make_modgate(type, cur_name, overrides, cur.parms, cur.range[0],
                         cur.range[1], cur.file, cur.lineno);

    } else {
      svector<PExpr *> *wires = new svector<PExpr *>(0);
      pform_make_modgate(type, cur_name, overrides, wires, cur.range[0],
                         cur.range[1], cur.file, cur.lineno);
    }
  }
...
void pform_make_pgassign_list(svector<PExpr *> *alist, svector<PExpr *> *del,
                              struct str_pair_t str, const char *fn,
                              unsigned lineno) {
  PGAssign *tmp;
  for (unsigned idx = 0; idx < alist->count() / 2; idx += 1) {
    tmp =
        pform_make_pgassign((*alist)[2 * idx], (*alist)[2 * idx + 1], del, str);
 ...
 * BTF-B14.
 */
void pform_make_reginit(const struct vlltype &li, perm_string name,
                        PExpr *expr) {
...
void pform_module_define_port(const struct vlltype &li, perm_string name,
                              NetNet::PortType port_type, NetNet::Type type,
                              SecType *sec_type, BaseType *base_type,
                              bool signed_flag, svector<PExpr *> *range,
                              svector<named_pexpr_t *> *attr) {
  PWire *cur = pform_get_wire_in_scope(name);
  if (cur) {
    ostringstream msg;
  ...
  if (range == 0) {
    cur->set_range(0, 0, (type == NetNet::IMPLICIT) ? SR_PORT : SR_BOTH, true);
    ...
    cur->set_range((*range)[0], (*range)[1],
                   (type == NetNet::IMPLICIT) ? SR_PORT : SR_BOTH, false);
  }
...
void pform_makewire(const vlltype &li, perm_string name, NetNet::Type type,
                    ...
                    svector<named_pexpr_t *> *attr, SecType *st, BaseType *bt) {
  PWire *cur = pform_get_wire_in_scope(name);
 ...
 */
void pform_makewire(const vlltype &li, svector<PExpr *> *range,
                    bool signed_flag, list<perm_string> *names,
                    NetNet::Type type, NetNet::PortType pt,
                    ivl_variable_type_t dt, svector<named_pexpr_t *> *attr,
                    PWSRType rt) {
  pform_makewire(li, range, signed_flag, names, type, pt, dt, new ConstType(),
                 new BaseType(), attr, rt);
}

void pform_makewire(const vlltype &li, svector<PExpr *> *range,
                    bool signed_flag, list<perm_string> *names,
                    NetNet::Type type, NetNet::PortType pt,
                    ivl_variable_type_t dt, SecType *st, BaseType *bt,
                    svector<named_pexpr_t *> *attr, PWSRType rt) {
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
       cur++) {
    perm_string txt = *cur;
 ...
 */
void pform_makewire(const vlltype &li, svector<PExpr *> *range,
                    bool signed_flag, svector<PExpr *> *delay, str_pair_t str,
  ...
  while (first) {
    ...
    /* This has already been done for real variables. */
    if (dt != IVL_VT_REAL) {
      pform_set_net_range(first->name, range, signed_flag, dt, st, bt, SR_NET);
    }
...
void pform_makewire(const vlltype &li, svector<PExpr *> *range,
                    bool signed_flag, svector<PExpr *> *delay, str_pair_t str,
                    list<perm_string> *decls, NetNet::Type type,
                    ivl_variable_type_t dt, SecType *st, BaseType *bt) {
  ...
  for (sitr_t cur = decls->begin(); cur != decls->end(); cur++) {
    ...
    /* This has already been done for real variables. */
    if (dt != IVL_VT_REAL) {
      pform_set_net_range(*cur, range, signed_flag, dt, st, bt, SR_NET);
    }
  }
}
...
void pform_set_port_type(perm_string name, NetNet::PortType pt, SecType *st,
  ...
  if (cur == 0) {
    ...
    // IVL_VT_NO_TYPE);
    cur = new PWire(name, NetNet::IMPLICIT, NetNet::PIMPLICIT, st, bt,
                    IVL_VT_NO_TYPE);
 ...
 */
svector<PWire *> *
pform_make_task_ports(NetNet::PortType pt, ivl_variable_type_t vtype,
                      bool signed_flag, svector<PExpr *> *range,
                      list<perm_string> *names, const char *file,
                      unsigned lineno, bool isint) {
  assert(names);
  ...
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
    ...
    if (curw) {
      ...
      curw = new PWire(name, NetNet::IMPLICIT_REG, pt, new ConstType(),
                       new BaseType(), vtype);
    ...
    if (isint)
      assert(curw->set_wire_type(NetNet::INTEGER));
...
LexicalScope::range_t *
pform_parameter_value_range(bool exclude_flag, bool low_open, PExpr *low_expr,
...
void pform_set_parameter(const struct vlltype &loc, perm_string name,
  ...
  if (scope->localparams.find(name) != scope->localparams.end()) {
    ...
    FILE_NAME(&tloc, loc);
    cerr << tloc.get_fileline()
         << ": error: localparam and "
            "parameter in '"
         << pform_cur_module->mod_name() << "' have the same name '" << name
         << "'." << endl;
    error_count += 1;
  }
...
void pform_set_localparam(const struct vlltype &loc, perm_string name,
  ...
  if (debug_typecheck) {
    ...
    // cerr << "sanity check " << pform_get_cur_scope()->localparams[name] <<
    // endl;
  }
...
extern PSpecPath *pform_assign_path_delay(PSpecPath *path,
                                          svector<PExpr *> *del) {
  if (path == 0)
    return 0;
...
void pform_set_port_type(const struct vlltype &li, list<perm_string> *names,
                         svector<PExpr *> *range, bool signed_flag,
                         NetNet::PortType pt, SecType *st, BaseType *bt) {
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
       cur++) {
    perm_string txt = *cur;
    pform_set_port_type(txt, pt, st, bt, li.text, li.first_line);
    pform_set_net_range(txt, range, signed_flag, IVL_VT_NO_TYPE, st, bt,
                        SR_PORT);
  }
...
static void pform_set_reg_integer(perm_string name, SecType *st, BaseType *bt) {
  PWire *cur = pform_get_wire_in_scope(name);
  if (cur == 0) {
    cur = new PWire(name, NetNet::INTEGER, NetNet::NOT_A_PORT, st, bt,
                    IVL_VT_LOGIC);
    cur->set_signed(true);
    pform_put_wire_in_scope(name, cur);
  } else {
  ...
  cur->set_range(new PENumber(new verinum(integer_width - 1, integer_width)),
                 new PENumber(new verinum((uint64_t)0, integer_width)), SR_NET,
                 false);
  cur->set_signed(true);
  cur->set_sec_type(st);
  cur->set_base_type(bt);
...
static void pform_set_reg_time(perm_string name, SecType *st, BaseType *bt) {
  ...
  if (cur == 0) {
    cur =
        new PWire(name, NetNet::REG, NetNet::NOT_A_PORT, st, bt, IVL_VT_LOGIC);
    pform_put_wire_in_scope(name, cur);
  } else {
...
svector<PWire *> *pform_make_udp_input_ports(list<perm_string> *names) {
  ...
  for (list<perm_string>::iterator cur = names->begin(); cur != names->end();
    ...
    PWire *pp       = new PWire(txt, NetNet::IMPLICIT, NetNet::PINPUT,
                                new ConstType(), new BaseType(), IVL_VT_LOGIC);
    (*out)[idx]     = pp;
    idx += 1;
  }
...
void pform_error_nested_modules() {
  assert(pform_cur_module != 0);
  cerr << pform_cur_module->get_fileline()
       << ": error: original module "
          "("
