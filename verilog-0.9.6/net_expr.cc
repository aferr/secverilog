 ...
 */
ivl_variable_type_t NetExpr::expr_type() const { return IVL_VT_LOGIC; }
...
NetEBAdd::NetEBAdd(char op__, NetExpr *l, NetExpr *r, bool lossless_flag)
  ...
  if ((tmp = dynamic_cast<NetEConst *>(r)) && (!tmp->has_width()) &&
  ...
  } else if ((tmp = dynamic_cast<NetEConst *>(l)) && (!tmp->has_width()) &&
             (tmp->expr_width() > r->expr_width() ||
              integer_width > r->expr_width())) {
...
NetEBAdd *NetEBAdd::dup_expr() const {
  NetEBAdd *result = new NetEBAdd(op_, left_->dup_expr(), right_->dup_expr());
  return result;
}
...
NetEBComp::NetEBComp(char op__, NetExpr *l, NetExpr *r)
  ...
  if (NetEConst *tmp = dynamic_cast<NetEConst *>(r))
    do {
  ...
  if (NetEConst *tmp = dynamic_cast<NetEConst *>(l))
    do {
...
NetEBDiv::NetEBDiv(char op__, NetExpr *l, NetExpr *r) : NetEBinary(op__, l, r) {
...
NetEBDiv *NetEBDiv::dup_expr() const {
  NetEBDiv *result = new NetEBDiv(op_, left_->dup_expr(), right_->dup_expr());
  return result;
}
...
NetEBMult *NetEBMult::dup_expr() const {
  NetEBMult *result = new NetEBMult(op_, left_->dup_expr(), right_->dup_expr());
...
NetEBPow::NetEBPow(char op__, NetExpr *l, NetExpr *r) : NetEBinary(op__, l, r) {
  assert(op__ == 'p');
...
NetEBPow *NetEBPow::dup_expr() const {
  NetEBPow *result = new NetEBPow(op_, left_->dup_expr(), right_->dup_expr());
...
NetEBShift *NetEBShift::dup_expr() const {
  NetEBShift *result =
      new NetEBShift(op_, left_->dup_expr(), right_->dup_expr());
  return result;
}

NetEConcat::NetEConcat(unsigned cnt, NetExpr *r) : parms_(cnt), repeat_(r) {
  if (repeat_ == 0) {
...
unsigned NetEConcat::repeat() {
  ...
  if (repeat_const == 0) {
    ...
    cerr << get_fileline() << ":               : Expression is: " << *repeat_
         << endl;
    repeat_calculated_ = true;
    repeat_value_      = 1;
    return 1;
...
bool NetECReal::has_width() const { return false; }
...
NetEParam::NetEParam() : des_(0), scope_(0) { solving_ = false; }
...
bool NetEParam::has_width() const { return false; }
...
void NetEParam::solving(bool arg) { solving_ = arg; }

bool NetEParam::solving() const { return solving_; }
...
unsigned NetESFunc::nparms() const { return nparms_; }
