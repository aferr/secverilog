...
NetExpr *elaborate_rval_expr(Design *des, NetScope *scope,
  ...
  int expr_wid = expr->test_width(des, scope, use_min_wid, use_lval_wid,
                                  rval_type, unsized_flag);

  if (debug_elaborate) {
    cerr << expr->get_fileline() << ": debug: r-value tested "
         << "type=" << rval_type << ", width=" << expr_wid
         << ", min=" << use_min_wid
 ...
 */
unsigned PExpr::test_width(Design *des, NetScope *scope, unsigned min,
                           unsigned lval, ivl_variable_type_t &, bool &) {
  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: test_width defaults to " << min
         << ", ignoring unsized_flag. typeid=" << typeid(*this).name() << endl;
  }
  return min;
}
...
unsigned PEBinary::test_width(Design *des, NetScope *scope, unsigned min,
  ...
  bool flag_left = flag;
  unsigned wid_left =
      left_->test_width(des, scope, min, 0, expr_type_left, flag_left);

  bool flag_right = flag;
  unsigned wid_right =
      right_->test_width(des, scope, min, 0, expr_type_right, flag_right);

  if (flag_right && !flag) {
    unsized_flag = true;
    wid_left =
        left_->test_width(des, scope, min, 0, expr_type_left, unsized_flag);
  }

  if (flag_left && !flag) {
    unsized_flag = true;
    wid_right =
        right_->test_width(des, scope, min, 0, expr_type_right, unsized_flag);
  }
 ...
 * types.
 */
NetExpr *PEBinary::elaborate_expr(Design *des, NetScope *scope, int expr_wid,
                                  bool) const {
  assert(left_);
  assert(right_);
  ...
  // lossless.
  if (lp->expr_type() == IVL_VT_REAL && type_is_vectorable(rp->expr_type()) &&
      expr_wid != -2) {
    delete rp;
    rp = right_->elaborate_expr(des, scope, -2, false);
  }

  if (rp->expr_type() == IVL_VT_REAL && type_is_vectorable(lp->expr_type()) &&
      expr_wid != -2) {
    delete lp;
...
NetExpr *PEBinary::elaborate_eval_expr_base_(Design *des, NetExpr *lp,
                                             NetExpr *rp, int expr_wid) const {
...
NetExpr *PEBinary::elaborate_expr_base_lshift_(Design *des, NetExpr *lp,
                                               NetExpr *rp,
                                               int expr_wid) const {
  if (lp->expr_type() == IVL_VT_REAL || rp->expr_type() == IVL_VT_REAL) {
    cerr << get_fileline() << ": error: " << human_readable_op(op_)
  ...
  if (NetEConst *lpc = dynamic_cast<NetEConst *>(lp)) {
    ...
    if (NetEConst *rpc = dynamic_cast<NetEConst *>(rp)) {
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Precalculate " << *lpc << " << " << shift << " to constant "
             << *tmp << " (expr_wid=" << expr_wid << ")" << endl;
    ...
    if (shift >= use_wid || (-shift) >= (long)lp->expr_width()) {
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Value left-shifted " << shift << " beyond width of " << use_wid
             << ". Elaborate as constant zero." << endl;
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Left shift expression by constant " << shift
             << " bits. (use_wid=" << use_wid << ")" << endl;
...
NetExpr *PEBinary::elaborate_expr_base_rshift_(Design *des, NetExpr *lp,
                                               NetExpr *rp,
                                               int expr_wid) const {
  if (lp->expr_type() == IVL_VT_REAL || rp->expr_type() == IVL_VT_REAL) {
    cerr << get_fileline() << ": error: " << human_readable_op(op_)
  ...
  if (NetEConst *rpc = dynamic_cast<NetEConst *>(rp)) {
    ...
    if ((op_ == 'r' || (lp->has_sign() == false)) &&
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Value right-shifted " << shift << " beyond width of "
             << lp->expr_width() << ". Elaborate as constant zero." << endl;
    ...
    if (shift >= lp->expr_width()) {
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             ...
             << lp->expr_width() << ". Elaborate as replicated top bit."
             << endl;
...
NetExpr *PEBinary::elaborate_expr_base_mult_(Design *des, NetExpr *lp,
                                             NetExpr *rp, int expr_wid,
                                             bool is_pexpr) const {
...
NetExpr *PEBinary::elaborate_expr_base_add_(Design *des, NetExpr *lp,
                                            NetExpr *rp, int expr_wid,
                                            bool is_pexpr) const {
  NetExpr *tmp;
...
unsigned PEBComp::test_width(Design *des, NetScope *scope, unsigned, unsigned,
                             ivl_variable_type_t &my_expr_type, bool &) {
  ...
  unsigned left_width =
      left_->test_width(des, scope, 0, 0, left_type, unsized_flag);
  bool left_unsized_flag         = unsized_flag;
  ivl_variable_type_t right_type = IVL_VT_NO_TYPE;
  unsigned right_width =
      right_->test_width(des, scope, 0, 0, right_type, unsized_flag);
  ...
  if (type_is_vectorable(left_type) && unsized_flag &&
      try_wid_l < (int)integer_width)
    try_wid_l += 1;
  if (type_is_vectorable(right_type) && unsized_flag &&
      try_wid_r < (int)integer_width)
    try_wid_r += 1;

  if (debug_elaborate) {
    cerr << get_fileline() << ": debug: "
         << "Comparison expression operands are " << left_width << " bits and "
         << right_width << " bits. Resorting to " << try_wid_l << " bits and "
         << try_wid_r << " bits." << endl;
  }
...
NetExpr *PEBComp::elaborate_expr(Design *des, NetScope *scope,
  ...
  if (flag == false) {
    cerr << get_fileline()
         << ": internal error: "
            "expression bit width of comparison != 1."
         << endl;
    des->errors += 1;
  }
...
unsigned PEBLogic::test_width(Design *des, NetScope *scope, unsigned min,
                              unsigned lval, ivl_variable_type_t &expr_type_out,
                              bool &unsized_flag) {
  expr_type_    = IVL_VT_LOGIC;
  expr_width_   = 1;
...
unsigned PEBLeftWidth::test_width(Design *des, NetScope *scope, unsigned min,
                                  unsigned lval,
                                  ivl_variable_type_t &expr_type__,
                                  bool &unsized_flag) {
  unsigned wid_left =
      left_->test_width(des, scope, min, lval, expr_type__, unsized_flag);
  ...
  if (unsized_flag && type_is_vectorable(expr_type__) && wid_left > 0 &&
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: "
           ...
           << " is padded to compiler integer width=" << wid_left << endl;
  }
...
NetExpr *PEBLeftWidth::elaborate_expr(Design *des, NetScope *scope,
  ...
  if (expr_wid > 0 && lp->expr_width() < (unsigned)expr_wid) {
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: "
           << "Pad left operand of " << human_readable_op(op_) << " to "
           << expr_wid << "." << endl;
    lp = pad_to_width(lp, expr_wid, *this);
  }
  ...
  if (name == "$signed" || name == "$unsigned") {
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: $signed/$unsigned"
           << " argument width = " << expr_width_ << ", result width = " << min
           << "." << endl;
  ...
  for (unsigned idx = 0; idx < parms_.size(); idx += 1) {
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: test_width"
           ...
           << " returns type=" << sub_type << ", wid=" << wid << endl;
  }
...
unsigned PECallFunction::test_width(Design *des, NetScope *scope, unsigned min,
                                    unsigned lval,
                                    ivl_variable_type_t &expr_type__,
                                    bool &unsized_flag) {
...
NetExpr *PECallFunction::cast_to_width_(NetExpr *expr, int wid,
  ...
  if (NetEConst *tmp = dynamic_cast<NetEConst *>(expr)) {
    ...
    if (!tmp->has_width() && tmp->has_sign() && (wid > 0)) {
      ...
      if ((unsigned)wid > integer_width) {
        if (integer_width > pwidth)
          pwidth = integer_width;
      } else {
        if ((unsigned)wid > pwidth)
          pwidth = wid;
      }
      tmp = dynamic_cast<NetEConst *>(pad_to_width(tmp, pwidth, *expr));
      assert(tmp);
    }
  ...
  if (debug_elaborate)
    cerr << get_fileline() << ": debug: cast to " << wid << " bits" << endl;
...
NetExpr *PECallFunction::elaborate_sfunc_(Design *des, NetScope *scope,
  ...
  NetESFunc *fun =
      new NetESFunc(peek_tail_name(path_), sfunc_type, wid, nparms);
  fun->set_line(*this);
  ...
  if (missing_parms > 0) {
    cerr << get_fileline() << ": error: The function " << peek_tail_name(path_)
    ...
    if (tmp) {
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug:"
             ...
             << " argwid=" << parms[idx]->expr_width() << ": " << *parms[idx]
             << endl;
...
unsigned PEConcat::test_width(Design *des, NetScope *scope, unsigned min,
                              unsigned lval, ivl_variable_type_t &expr_type__,
                              bool &unsized_flag) {
  expr_type_ = IVL_VT_LOGIC;
  ...
  for (unsigned idx = 0; idx < parms_.count(); idx += 1) {
    tested_widths_[idx] =
        parms_[idx]->test_width(des, scope, 0, 0, expr_type__, unsized_flag);
  ...
  if (repeat_) {
    ...
    if (NetEConst *tmp_c = dynamic_cast<NetEConst *>(tmp)) {
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "CONCAT MISSING TEST_WIDTH WHEN REPEAT IS PRESENT!" << endl;
    }
...
NetExpr *PEConcat::elaborate_expr(Design *des, NetScope *scope, int expr_wid,
                                  bool) const {
  concat_depth += 1;
  NetExpr *repeat = 0;
  ...
  for (unsigned idx = 0; idx < parms_.count(); idx += 1) {
    ...
    assert(parms_[idx]);
    NetExpr *ex =
        elab_and_eval(des, scope, parms_[idx], tested_widths_[idx], 0);
    if (ex == 0)
      continue;
 ...
 */
unsigned PEFNumber::test_width(Design *des, NetScope *scope, unsigned min,
                               unsigned lval, ivl_variable_type_t &expr_type__,
                               bool &unsized_flag) {
  expr_type_   = IVL_VT_REAL;
  expr_width_  = 1;
...
NetExpr *PEFNumber::elaborate_expr(Design *des, NetScope *scope, int,
                                   bool) const {
...
bool PEIdent::calculate_parts_(Design *des, NetScope *scope, long &msb,
  ...
  bool tmp_flag                = false;
  int msb_wid =
      index_tail.msb->test_width(des, scope, 0, 0, tmp_type, tmp_flag);

  tmp_type = IVL_VT_NO_TYPE;
  tmp_flag = false;
  int lsb_wid =
      index_tail.lsb->test_width(des, scope, 0, 0, tmp_type, tmp_flag);
...
unsigned PEIdent::test_width(Design *des, NetScope *scope, unsigned min,
                             unsigned lval, ivl_variable_type_t &expr_type__,
                             bool &unsized_flag) {
  NetNet *net        = 0;
  const NetExpr *par = 0;
 ...
 */
NetExpr *PEIdent::elaborate_expr(Design *des, NetScope *scope, int expr_wid,
                                 bool sys_task_arg) const {
  assert(scope);
  ...
  NetScope *found_in =
      symbol_search(this, des, scope, path(), net, par, eve, ex1, ex2);
  ...
  // set in the scope.
  if (path().size() == 1 && scope->genvar_tmp.str() &&
      strcmp(peek_tail_name(path()), scope->genvar_tmp) == 0) {
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: " << path()
           << " is genvar with value " << scope->genvar_tmp_val << "." << endl;
  ...
  if (path().size() == 1 &&
    ...
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: " << path() << " is a specparam"
           << endl;
    return tmp;
  }

  if (error_implicit == false && sys_task_arg == false && path().size() == 1 &&
    ...
    if (warn_implicit) {
      cerr << get_fileline()
           << ": warning: implicit "
              "definition of wire "
           << scope_path(scope) << "." << peek_tail_name(path()) << "." << endl;
    }
...
NetExpr *PEIdent::elaborate_expr_param_(Design *des, NetScope *scope,
  ...
  if (use_sel == index_component_t::SEL_BIT) {
    ...
    if (le && re) {
      ...
      if (!re->value().is_defined()) {
        if (warn_ob_select) {
          ...
          cerr << get_fileline()
               ...
               << re->value() << "] for parameter '" << name << "'." << endl;
          cerr << get_fileline()
               << ":        : "
      ...
      if (par_me) {
        mtmp = normalize_variable_base(mtmp, par_me->value().as_long(),
                                       par_le->value().as_long(), 1, true);
      }
    ...
    if (ctmp != 0) {
      perm_string name     = peek_tail_name(path());
      NetEConstParam *ptmp = new NetEConstParam(found_in, name, ctmp->value());
      ...
      if (debug_elaborate)
        cerr << get_fileline() << ": debug: "
             << "Elaborate parameter <" << name << "> as constant " << *ptmp
             << endl;
      delete tmp;
      tmp = ptmp;
    }
    ...
    if (rtmp != 0) {
      perm_string name     = peek_tail_name(path());
      NetECRealParam *ptmp = new NetECRealParam(found_in, name, rtmp->value());
...
NetExpr *PEIdent::elaborate_expr_net_word_(Design *des, NetScope *scope,
  ...
  if (NetEConst *word_addr = dynamic_cast<NetEConst *>(word_index)) {
    ...
    if (long base = net->array_first()) {
      ...
      word_index =
          normalize_variable_array_base(word_index, base, net->array_count());
  ...
  if (net->get_scalar() && word_sel != index_component_t::SEL_NONE) {
    ...
    if (res->expr_type() == IVL_VT_REAL)
      ...
      cerr << "scalar";
    cerr << " array word: " << net->name() << "[" << *word_index << "]" << endl;
    des->errors += 1;
    delete res;
    return 0;
  }
...
NetExpr *PEIdent::elaborate_expr_net_idx_up_(Design *des, NetScope *scope,
  ...
  if (NetEConst *base_c = dynamic_cast<NetEConst *>(base)) {
    ...
    if (base_c->value().is_defined()) {
      ...
      // range.
      ex = new NetEConst(verinum(net->sig()->sb_to_idx(lsv) + offset));
      if (warn_ob_select) {
...
NetExpr *PEIdent::elaborate_expr_net_bit_(Design *des, NetScope *scope,
  ...
  ex = normalize_variable_base(ex, net->sig()->msb(), net->sig()->lsb(), 1,
                               true);
...
unsigned PENumber::test_width(Design *, NetScope *, unsigned min, unsigned lval,
...
NetEConst *PENumber::elaborate_expr(Design *des, NetScope *, int expr_width__,
                                    bool) const {
  assert(value_);
...
unsigned PETernary::test_width(Design *des, NetScope *scope, unsigned min,
                               unsigned lval,
                               ivl_variable_type_t &use_expr_type, bool &flag) {
  ...
  if (initial_flag == false && flag == true) {
    if (debug_elaborate)
      cerr << get_fileline() << ": debug: "
           ...
           << endl;
    tru_wid =
        tru_->test_width(des, scope, max(min, fal_wid), lval, tru_type, flag);
  }
  ...
  if (debug_elaborate)
    cerr << get_fileline() << ": debug: "
         ...
         << ", width=" << expr_width_ << ", unsized_flag=" << flag
         << " (tru_type=" << tru_type << ", fal_type=" << fal_type << ")"
         << endl;
 ...
 */
NetExpr *PETernary::elaborate_expr(Design *des, NetScope *scope, int expr_wid,
                                   bool) const {
  assert(expr_);
  assert(tru_);
  assert(fal_);
  ...
  if (NetEConst *tmp = dynamic_cast<NetEConst *>(con)) {
    ...
    if (cval.get(0) == verinum::V1) {
      ...
      ivl_assert(*this, use_wid > 0);
      NetExpr *texpr = elab_and_eval_alternative_(des, scope, tru_, use_wid);
    ...
    if (cval.get(0) == verinum::V0) {
      ...
      ivl_assert(*this, use_wid > 0);
      NetExpr *texpr = elab_and_eval_alternative_(des, scope, fal_, use_wid);
      return pad_to_width(texpr, use_wid, *this);
    }
...
unsigned PEUnary::test_width(Design *des, NetScope *scope, unsigned min,
                             unsigned lval, ivl_variable_type_t &expr_type__,
                             bool &unsized_flag) {
  switch (op_) {
  case '&': // Reduction AND
  ...
  unsigned test_wid =
      expr_->test_width(des, scope, min, lval, expr_type__, unsized_flag);
  switch (op_) {
...
NetExpr *PEUnary::elaborate_expr(Design *des, NetScope *scope, int expr_wid,
                                 bool) const {
