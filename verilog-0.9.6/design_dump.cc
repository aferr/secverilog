...
void NetCastInt::dump_node(ostream &o, unsigned ind) const {
  o << setw(ind) << ""
    << "Cast to int. (NetCastInt): " << name() << " width=" << width() << endl;
  dump_node_pins(o, ind + 4);
  dump_obj_attr(o, ind + 4);
}
...
void NetAnalogTop::dump(ostream &o, unsigned ind) const {
  switch (type_) {
  case IVL_PR_INITIAL:
    o << "analog initial /* " << get_fileline() << " in " << scope_path(scope_)
      << " */" << endl;
    break;
...
void NetEvProbe::dump_node(ostream &o, unsigned ind) const {
  ...
  o << setw(ind) << ""
    << "-> " << scope_path(event_->scope()) << "." << event_->name() << "; "
    << endl;
  dump_node_pins(o, ind + 4);
  dump_obj_attr(o, ind + 4);
}
...
void NetFuncDef::dump(ostream &o, unsigned ind) const {
  o << setw(ind) << ""
    << "function definition for " << scope_path(scope_) << endl;
  if (result_sig_) {
    o << setw(ind + 2) << ""
      << "Return signal: ";
    if (result_sig_->get_signed())
      o << "+";
  ...
  for (unsigned idx = 0; idx < port_count(); idx += 1) {
    ...
    if (port(idx)->get_signed())
      o << "+";
    o << port(idx)->name() << "[" << port(idx)->msb() << ":" << port(idx)->lsb()
      << "]" << endl;
  }
  if (statement_)
    statement_->dump(o, ind + 2);
...
void NetScope::dump(ostream &o) const {
  ...
  /* Dump the parameters for this scope. */
  {
    ...
    for (pp = localparams.begin(); pp != localparams.end(); pp++) {
      o << "    localparam " << (*pp).first << " = " << *(*pp).second.expr
        << ";" << endl;
    }
  }
