  ...
  for (iword = 0; iword < ivl_signal_array_count(sig); iword += 1) {
    ...
    if (nex_data->net == 0) {
      ...
      fprintf(vvp_out, "v%p_%u .alias%s \"%s\", %d %d, v%p_%u;\n", sig, iword,
              datatype_flag, vvp_mangle_name(ivl_signal_basename(sig)), msb,
              lsb, nex_data->net, nex_data->net_word);
    }
  }
}
...
static void draw_delay(ivl_net_logic_t lptr) {
  ...
  if (d0 == d1 && d1 == d2)
    ...
    fprintf(vvp_out, " (%lu,%lu,%lu)", get_number_immediate(d0),
            get_number_immediate(d1), get_number_immediate(d2));
}
...
static void draw_udp_in_scope(ivl_net_logic_t lptr) {
  ...
  fprintf(vvp_out, " UDP_%s", vvp_mangle_id(ivl_udp_name(udp)));
  draw_delay(lptr);
...
static void draw_logic_in_scope(ivl_net_logic_t lptr) {
  ...
  while (ninp) {
    ...
    for (inst = 0; inst < (unsigned)ninp; inst += 4) {
      if (ninp > 4)
        fprintf(vvp_out, "L_%p/%d/%d .functor %s %u", lptr, level, inst, lcasc,
                vector_width);
      else {
      ...
      for (pdx = inst; pdx < (unsigned)ninp && pdx < inst + 4; pdx += 1) {
        if (level) {
          fprintf(vvp_out, ", L_%p/%d/%d", lptr, level - 1, pdx * 4);
        } else {
          fprintf(vvp_out, ", %s", input_strings[pdx]);
        }
      }
  ...
  if (need_delay_flag) {
    ...
    if (number_is_immediate(rise_exp, 64, 0) &&
      ...
      fprintf(vvp_out,
              "L_%p .delay (%" PRIu64 ",%" PRIu64 ",%" PRIu64 ") L_%p/d;\n",
...
static const char *draw_lpm_output_delay(ivl_lpm_t net) {
  ...
  if (d_rise != 0) {
    ...
    dly = "/d";
    fprintf(vvp_out,
            "L_%p .delay (%" PRIu64 ",%" PRIu64 ",%" PRIu64 ")"
            " L_%p/d;\n",
            net, get_number_immediate64(d_rise), get_number_immediate64(d_fall),
  ...
  if (icnt <= 4) {
    ...
    while (icnt > 4) {
      for (idx = 0; idx < icnt; idx += 4) {
        ...
        fprintf(vvp_out, "LS_%p_%u_%u .concat [", net, depth, idx);
...
static void draw_lpm_part(ivl_lpm_t net) {
  ...
  if (sel == 0) {
    ...
    if (ivl_lpm_signed(net) && width_of_nexus(sel) < 8 * sizeof(int)) {
      fprintf(stderr,
              "%s:%u: tgt-vvp warning: V0.9 may give "
...
#endif
              ivl_lpm_file(net), ivl_lpm_lineno(net), 8 * sizeof(int));
    }
    fprintf(vvp_out, "L_%p%s .part/v %s", net, dly,
            draw_net_input(ivl_lpm_data(net, 0)));
...
int draw_scope(ivl_scope_t net, ivl_scope_t parent) {
  ...
  fprintf(vvp_out, "S_%p .scope %s%s, \"%s\" \"%s\" %d %d", net, prefix, type,
          vvp_mangle_name(ivl_scope_basename(net)), ivl_scope_tname(net),
  ...
  for (idx = 0; idx < ivl_scope_params(net); idx += 1) {
    ...
    case IVL_EX_STRING:
      ...
      break;
    case IVL_EX_NUMBER:
      fprintf(vvp_out, "P_%p .param/l \"%s\" %d %d, %sC4<", par,
              ivl_parameter_basename(par),
              ivl_file_table_index(ivl_parameter_file(par)),
