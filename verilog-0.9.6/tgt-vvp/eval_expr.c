...
int number_is_immediate(ivl_expr_t expr, unsigned lim_wid,
  ...
  if (ivl_expr_type(expr) == IVL_EX_ULONG) {
    unsigned long imm;
    if (lim_wid >= 8 * sizeof(unsigned long))
      return 1;
...
static struct vector_info draw_binary_expr_eq(ivl_expr_t expr, unsigned ewid,
                                              int stuff_ok_flag) {
...
static struct vector_info draw_binary_expr_lor(ivl_expr_t expr, unsigned wid,
  ...
  if (wid == 1 &&
      (lv.base < 4 || lv.base >= 8 || (stuff_ok_flag & STUFF_OK_47)))
    return lv;
...
static struct vector_info draw_binary_expr_le(ivl_expr_t expr, unsigned wid,
                                              int stuff_ok_flag) {
  ivl_expr_t le = ivl_expr_oper1(expr);
  ivl_expr_t re = ivl_expr_oper2(expr);
  ...
  if (ivl_expr_value(le) == IVL_VT_BOOL && ivl_expr_value(re) == IVL_VT_BOOL &&
      owid < 64) {
...
static struct vector_info draw_logic_immediate(ivl_expr_t expr, ivl_expr_t le,
                                               ivl_expr_t re, unsigned wid) {
    ...
    if (lv.base < 4) {
      ...
      if (tmp.base == 0) {
        fprintf(stderr,
                ...
                ivl_expr_file(expr), ivl_expr_lineno(expr), lv.wid);
        vvp_errors += 1;
      }
...
static struct vector_info draw_load_add_immediate(ivl_expr_t le, ivl_expr_t re,
                                                  unsigned wid,
                                                  int signed_flag) {
  struct vector_info lv;
  long imm;
...
static struct vector_info draw_add_immediate(ivl_expr_t le, ivl_expr_t re,
                                             unsigned wid) {
  struct vector_info lv;
  unsigned long imm;
 ...
 */
static struct vector_info draw_sub_immediate(ivl_expr_t le, ivl_expr_t re,
                                             unsigned wid) {
  struct vector_info lv;
  unsigned long imm;
  unsigned tmp;
...
static struct vector_info draw_mul_immediate(ivl_expr_t le, ivl_expr_t re,
                                             unsigned wid) {
  struct vector_info lv;
  unsigned long imm;
...
static struct vector_info draw_binary_expr_arith(ivl_expr_t expr,
                                                 unsigned wid) {
  ivl_expr_t le = ivl_expr_oper1(expr);
  ivl_expr_t re = ivl_expr_oper2(expr);
  ...
  if ((ivl_expr_opcode(expr) == '+') && (ivl_expr_type(le) == IVL_EX_SIGNAL) &&
      (ivl_expr_type(re) == IVL_EX_ULONG) &&
      number_is_immediate(re, IMM_WID, 1))
    return draw_load_add_immediate(le, re, wid, signed_flag);

  if ((ivl_expr_opcode(expr) == '+') && (ivl_expr_type(le) == IVL_EX_SIGNAL) &&
      (ivl_expr_type(re) == IVL_EX_NUMBER) && (!number_is_unknown(re)) &&
      number_is_immediate(re, IMM_WID, 1))
    return draw_load_add_immediate(le, re, wid, signed_flag);

  if ((ivl_expr_opcode(expr) == '+') && (ivl_expr_type(re) == IVL_EX_SIGNAL) &&
      (ivl_expr_type(le) == IVL_EX_ULONG) &&
      number_is_immediate(re, IMM_WID, 1))
    return draw_load_add_immediate(re, le, wid, signed_flag);

  if ((ivl_expr_opcode(expr) == '+') && (ivl_expr_type(re) == IVL_EX_SIGNAL) &&
      (ivl_expr_type(le) == IVL_EX_NUMBER) && (!number_is_unknown(le)) &&
      number_is_immediate(le, IMM_WID, 1))
    return draw_load_add_immediate(re, le, wid, signed_flag);

  if ((ivl_expr_opcode(expr) == '+') && (ivl_expr_type(re) == IVL_EX_ULONG) &&
      number_is_immediate(re, IMM_WID, 0))
    return draw_add_immediate(le, re, wid);

  if ((ivl_expr_opcode(expr) == '+') && (ivl_expr_type(re) == IVL_EX_NUMBER) &&
      (!number_is_unknown(re)) && number_is_immediate(re, IMM_WID, 0))
    return draw_add_immediate(le, re, wid);
  ...
  if (lv.wid != wid) {
    fprintf(stderr,
            "XXXX ivl_expr_opcode(expr) = %c,"
            " lv.wid=%u, wid=%u\n",
            ivl_expr_opcode(expr), lv.wid, wid);
  }
...
static struct vector_info draw_binary_expr(ivl_expr_t expr, unsigned wid,
                                           int stuff_ok_flag) {
  struct vector_info rv;
...
static struct vector_info draw_concat_expr(ivl_expr_t expr, unsigned wid,
  ...
  if (res.base == 0) {
    fprintf(stderr,
            ...
            ivl_expr_file(expr), ivl_expr_lineno(expr), rep ? wid : expr_wid);
    vvp_errors += 1;
  }
  ...
  if (concat_wid <= wid) {
    ...
    while (rep > 0) {
      fprintf(vvp_out, "    %%mov %u, %u, %u; Repetition %u\n",
              res.base + expr_wid * rep, res.base, expr_wid, rep + 1);
      rep -= 1;
    }
...
static struct vector_info draw_number_expr(ivl_expr_t expr, unsigned wid) {
  ...
  while (idx < nwid) {
    ...
    if (val_bits >= IMM_WID || (val_bits > 0 && src != 0) ||
 ...
 */
static void pad_in_place(struct vector_info dest, unsigned sub_width,
                         int signed_flag) {
  if (signed_flag) {
    unsigned idx;
    for (idx = sub_width; idx < dest.wid; idx += 1)
      fprintf(vvp_out, "    %%mov %u, %u, 1;\n", dest.base + idx,
              dest.base + sub_width - 1);
  } else {
    fprintf(vvp_out, "    %%mov %u, 0, %u;\n", dest.base + sub_width,
            dest.wid - sub_width);
  }
...
static struct vector_info draw_select_array(ivl_expr_t sube, ivl_expr_t bit_idx,
                                            unsigned bit_width, unsigned wid) {
  unsigned idx, label;
...
static struct vector_info draw_select_signal(ivl_expr_t expr, ivl_expr_t sube,
                                             ivl_expr_t bit_idx,
                                             unsigned bit_wid, unsigned wid) {
  ivl_signal_t sig = ivl_expr_signal(sube);
  struct vector_info res;
  ...
  /* Try the special case that the part is at the beginning of
     ...
     in one go. */
  if (number_is_immediate(bit_idx, IMM_WID, 0) && !number_is_unknown(bit_idx) &&
      get_number_immediate(bit_idx) == 0 && (ivl_expr_width(sube) >= bit_wid)) {
...
static void draw_select_signal_dest(ivl_expr_t expr, ivl_expr_t sube,
                                    ivl_expr_t bit_idx, struct vector_info dest,
                                    int stuff_ok_flag) {
  struct vector_info tmp;
  ivl_signal_t sig = ivl_expr_signal(sube);
  ...
  if ((ivl_signal_dimensions(sig) == 0) && (ivl_expr_width(sube) >= dest.wid) &&
      number_is_immediate(bit_idx, IMM_WID, 0) && !number_is_unknown(bit_idx) &&
      get_number_immediate(bit_idx) == 0) {
    unsigned use_word = 0;
    fprintf(vvp_out, "    %%load/v %u, v%p_%u, %u; Select %u out of %u bits\n",
            dest.base, sig, use_word, dest.wid, dest.wid, ivl_expr_width(sube));
    return;
  }
...
static struct vector_info draw_select_expr(ivl_expr_t expr, unsigned wid,
  ...
  if (ivl_expr_type(sube) == IVL_EX_SIGNAL) {
    res = draw_select_signal(expr, sube, shift, ivl_expr_width(expr), wid);
...
static struct vector_info draw_sfunc_expr(ivl_expr_t expr, unsigned wid) {
  ...
  if (parm_count == 0) {
    ...
    fprintf(vvp_out, "    %%vpi_func %u %u \"%s\", %u, %u;\n",
            ...
            ivl_expr_name(expr), res.base, res.wid);
    return res;
  }
...
struct vector_info draw_eval_expr_wid(ivl_expr_t expr, unsigned wid,
  ...
  case IVL_EX_NONE:
    fprintf(stderr,
            "%s:%u:  vvp.tgt error: unhandled expr. type: "
            "%u at %s:%d\n",
  ...
  case IVL_EX_EVENT:
    fprintf(stderr,
            "%s:%u: vvp.tgt error: A named event is not "
