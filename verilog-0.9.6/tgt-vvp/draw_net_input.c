    ...
    if (d_rise != 0) {
      ...
      if (number_is_immediate(d_rise, 64, 0) &&
        ...
        fprintf(vvp_out,
                ...
                get_number_immediate64(d_fall), get_number_immediate64(d_decay),
                cptr);
...
static char *draw_island_port(ivl_island_t island, ivl_nexus_t nex,
                              const char *src) {
  char result[64];
...
char *draw_net_input_x(ivl_nexus_t nex, ivl_nexus_ptr_t omit_ptr,
  ...
  while (ndrivers) {
    ...
    for (inst = 0; inst < ndrivers; inst += 4) {
      if (ndrivers > 4)
        fprintf(vvp_out, "RS_%p/%d/%d .resolv %s", nex, level, inst,
                branch_type);
      else
      ...
      for (idx = inst; idx < ndrivers && idx < inst + 4; idx += 1) {
        if (level) {
          fprintf(vvp_out, ", RS_%p/%d/%d", nex, level - 1, idx * 4);
        } else {
