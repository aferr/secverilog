...
static int get_vpi_taskfunc_signal_arg(struct args_info *result,
...
#endif
                  ivl_expr_file(expr), ivl_expr_lineno(expr), 8 * sizeof(int));
        }
        snprintf(buffer, sizeof buffer, "&A<v%p, %u %u>", sig, av.base, av.wid);
        result->vec      = av;
        result->vec_flag = 1;
      } else {
  ...
  case IVL_EX_SELECT: {
    ...
    if (number_is_immediate(bexpr, 64, 1)) {
      ...
      if (ivl_expr_signed(bexpr) && (ivl_expr_width(bexpr) < 8 * sizeof(int))) {
        fprintf(stderr,
      ...
      snprintf(buffer, sizeof buffer, "&PV<v%p_0, %u %u, %u>",
               ivl_expr_signal(vexpr), rv.base, rv.wid, ivl_expr_width(expr));
      result->vec      = rv;
      result->vec_flag = 1;
    }
    result->text = strdup(buffer);
...
static void draw_vpi_taskfunc_args(const char *call_string,
  ...
  for (idx = 0; idx < parm_count; idx += 1) {
    ...
    case IVL_VT_BOOL:
      ...
      snprintf(buffer, sizeof buffer, "T<%u,%u,%s>", args[idx].vec.base,
               args[idx].vec.wid, ivl_expr_signed(expr) ? "s" : "u");
      break;
    case IVL_VT_REAL:
      args[idx].vec_flag = 1;
      args[idx].vec.base = draw_eval_real(expr);
      args[idx].vec.wid  = 0;
      snprintf(buffer, sizeof buffer, "W<%u,r>", args[idx].vec.base);
      break;
    default:
      assert(0);
    }
