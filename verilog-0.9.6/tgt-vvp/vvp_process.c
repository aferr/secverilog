...
static void set_to_lvariable(ivl_lval_t lval, unsigned bit, unsigned wid) {
...
static void assign_to_lvector(ivl_lval_t lval, unsigned bit, uint64_t delay,
                              ivl_expr_t dexp, unsigned width,
                              unsigned nevents) {
  ivl_signal_t sig       = ivl_lval_sig(lval);
  ivl_expr_t part_off_ex = ivl_lval_part_off(lval);
  unsigned long part_off = 0;
    ...
    if (dexp != 0) {
      ...
      if (hig_d != 0) {
        ...
        fprintf(vvp_out, "    %%ix/load %d, %lu, %lu;\n", delay_index, low_d,
                hig_d);
        fprintf(vvp_out, "    %%assign/v0/x1/d v%p_%lu, %d, %u;\n", sig,
                use_word, delay_index, bit);
        clr_word(delay_index);
      } else {
        fprintf(vvp_out, "    %%assign/v0/x1 v%p_%lu, %lu, %u;\n", sig,
                use_word, low_d, bit);
      }
      fprintf(vvp_out, "t_%u ;\n", skip_assign);
    }
  ...
  if (ivl_expr_value(rval) == IVL_VT_REAL) {
    ...
    if (vec.base == 0) {
      fprintf(stderr,
              ...
              "r-value expression.\n",
              ivl_expr_file(rval), ivl_expr_lineno(rval), wid);
      vvp_errors += 1;
    }
...
static int show_stmt_assign_nb_real(ivl_statement_t net) {
  ...
  if (ivl_signal_dimensions(sig) > 0) {
    ...
    fprintf(stderr,
            "%s:%u: tgt-vvp sorry: non-blocking assignment "
...
static int show_stmt_case(ivl_statement_t net, ivl_scope_t sscope) {
  ...
  for (idx = 0; idx < count; idx += 1) {
    ...
    if ((ivl_statement_type(net) == IVL_ST_CASE) &&
        (ivl_expr_type(cex) == IVL_EX_NUMBER) && (!number_is_unknown(cex)) &&
 ...
 */
static int show_stmt_noop(ivl_statement_t net) { return 0; }
