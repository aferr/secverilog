...
NetNet *add_to_net(Design *des, NetNet *sig, long val) {
  if (val == 0)
    return sig;
      ...
      return res;
#else
  cerr << sig->get_fileline() << ": XXXX: Forgot how to implement add_to_net"
       << endl;
  return 0;
#endif
}
...
NetNet *sub_net_from(Design *des, NetScope *scope, long val, NetNet *sig) {
  NetNet *zero_net = new NetNet(scope, scope->local_symbol(), NetNet::WIRE,
                                sig->vector_width());
...
void eval_expr(NetExpr *&expr, int prune_width) {
  assert(expr);
  if (dynamic_cast<NetECReal *>(expr))
    return;
  ...
  if (NetEConst *tmp = dynamic_cast<NetEConst *>(expr)) {
    ...
    if (tmp->has_width())
      return;
    if ((unsigned)prune_width <= tmp->expr_width())
      return;
