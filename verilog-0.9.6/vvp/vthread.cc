...
static inline void thr_put_bit(struct vthread_s *thr, unsigned addr,
                               vvp_bit4_t val) {
...
vvp_bit4_t vthread_get_bit(struct vthread_s *thr, unsigned addr) {
  if (vpi_mode_flag == VPI_MODE_COMPILETF)
    return BIT4_X;
  else
...
static unsigned long *vector_to_array(struct vthread_s *thr, unsigned addr,
                                      unsigned wid) {
  if (addr == 0) {
 ...
 */
static vvp_vector4_t vthread_bits_to_vector(struct vthread_s *thr, unsigned bit,
                                            unsigned wid) {
 ...
 * in that scope.
 */
static void vthread_free_context(vvp_context_t context,
...
bool of_ASSIGN_V0X1E(vthread_t thr, vvp_code_t cp) {
  ...
  if (thr->ecount == 0) {
    ...
    schedule_evctl(ptr, value, off, sig->size(), thr->event, thr->ecount);
  }
...
bool of_ASSIGN_WRE(vthread_t thr, vvp_code_t cp) {
  ...
  if (thr->ecount == 0) {
    ...
    schedule_evctl(tmp, thr->words[index].w_real, thr->event, thr->ecount);
  }
...
bool of_BREAKPOINT(vthread_t thr, vvp_code_t cp) { return true; }
...
bool of_CASSIGN_LINK(vthread_t thr, vvp_code_t cp) {
  ...
  vvp_fun_signal_base *sig = reinterpret_cast<vvp_fun_signal_base *>(dst->fun);
  assert(sig);
...
static uint64_t vector_to_index(vthread_t thr, unsigned base, unsigned width,
                                bool signed_flag) {
  uint64_t v        = 0;
...
#endif
static void load_vp0_common(vthread_t thr, vvp_code_t cp,
                            const vvp_vector4_t &sig_value) {
  unsigned bit   = cp->bit_idx[0];
  unsigned wid   = cp->bit_idx[1];
...
static void do_verylong_mod(vthread_t thr, vvp_code_t cp, bool left_is_neg,
                            bool right_is_neg) {
...
bool of_MOD_S(vthread_t thr, vvp_code_t cp) {
  ...
  if (cp->number <= 8 * sizeof(long long)) {
    ...
    bool left_is_neg  = thr_get_bit(thr, cp->bit_idx[0] + cp->number - 1) == 1;
    bool right_is_neg = thr_get_bit(thr, cp->bit_idx[1] + cp->number - 1) == 1;
...
bool of_NOOP(vthread_t thr, vvp_code_t cp) { return true; }
...
bool of_POW(vthread_t thr, vvp_code_t cp) {
  ...
  for (unsigned jdx = 0; jdx < wid; jdx += 1)
    thr_put_bit(thr, cp->bit_idx[0] + jdx, result.value(jdx) ? BIT4_1 : BIT4_0);
...
bool of_SET_VEC(vthread_t thr, vvp_code_t cp) {
  ...
  vvp_send_vec4(ptr, vthread_bits_to_vector(thr, bit, wid), thr->wt_context);
