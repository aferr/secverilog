...
double vpip_time_to_scaled_real(vvp_time64_t ti, struct __vpiScope *scope) {
  ...
  int scale = 0;
  if (scope)
    scale = vpi_time_precision - scope->time_units;
...
static void timevar_get_value(vpiHandle ref, s_vpi_value *vp, bool is_int_func,
  ...
  case vpiRealVal:
    ...
    if (is_int_func)
      vp->value.real = double(simtime);
    /* This is a call to $realtime to return a real value so
     * scale this using the scaled real rules. */
    else
      vp->value.real = vpip_time_to_scaled_real(schedule_simtime(), rfp->scope);
    break;
...
static const struct __vpirt vpip_system_time_rt = {vpiSysFuncCall,
                                                   timevar_time_get,
...
static const struct __vpirt vpip_system_stime_rt = {vpiSysFuncCall,
                                                    timevar_stime_get,
...
static const struct __vpirt vpip_system_simtime_rt = {vpiSysFuncCall,
                                                      timevar_time_get,
...
static const struct __vpirt vpip_system_realtime_rt = {vpiSysFuncCall,
