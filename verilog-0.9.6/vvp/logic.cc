...
void vvp_fun_muxr::recv_vec4(vvp_net_ptr_t ptr, const vvp_vector4_t &bit,
  ...
  switch (bit.value(0)) {
  case BIT4_0:
    if (select_ == SEL_PORT0)
      return;
    select_ = SEL_PORT0;
    break;
  case BIT4_1:
    if (select_ == SEL_PORT1)
      return;
    select_ = SEL_PORT1;
    break;
  default:
...
void vvp_fun_muxr::recv_real(vvp_net_ptr_t ptr, double bit, vvp_context_t) {
  ...
  case 0:
    ...
    a_ = bit;
    if (select_ == SEL_PORT1)
      return; // The other port is selected.
    break;
...
vvp_fun_muxz::vvp_fun_muxz(unsigned wid) : a_(wid), b_(wid) {
  net_ = 0;
  count_functors_logic += 1;
  select_  = SEL_BOTH;
  has_run_ = false;
