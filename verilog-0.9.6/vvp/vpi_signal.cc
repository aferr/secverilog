...
char *generic_get_str(int code, vpiHandle ref, const char *name,
  ...
  if (index != NULL)
    len += strlen(index) + 2; /* include space for brackets */
...
static void format_vpiStringVal(vvp_fun_signal_vec *sig, int base, unsigned wid,
  ...
  for (long idx = base + (signed)wid - 1; idx >= base; idx -= 1) {
    ...
    if (idx >= 0 && idx < (signed)sig->size() && sig->value(idx) == BIT4_1) {
      tmp |= 1;
    }
...
static unsigned signal_width(const struct __vpiSignal *rfp) {
  unsigned wid = (rfp->msb >= rfp->lsb) ? (rfp->msb - rfp->lsb + 1)
                                        : (rfp->lsb - rfp->msb + 1);
...
static void signal_get_value(vpiHandle ref, s_vpi_value *vp) {
  ...
  default:
    fprintf(stderr,
            ...
            (int)vp->format, vpi_get_str(vpiName, ref), vpip_scope(rfp)->name);
    assert(0);
  }
}
...
static vpiHandle signal_put_value(vpiHandle ref, s_vpi_value *vp, int flags) {
  ...
  wid = (rfp->msb >= rfp->lsb) ? (rfp->msb - rfp->lsb + 1)
                               : (rfp->lsb - rfp->msb + 1);
...
static const struct __vpirt vpip_reg_rt = {
    vpiReg,           signal_get,        signal_get_str, signal_get_value,
    signal_put_value, signal_get_handle, signal_iterate, 0};

static const struct __vpirt vpip_net_rt = {
    vpiNet,           signal_get,        signal_get_str, signal_get_value,
    signal_put_value, signal_get_handle, signal_iterate, 0};
 ...
 */
vpiHandle vpip_make_reg(const char *name, int msb, int lsb, bool signed_flag,
                        vvp_net_t *vec) {
...
static struct __vpiSignal *allocate_vpiSignal(void) {
  ...
  if ((alloc_array == 0) || (alloc_index == alloc_count)) {
    alloc_array =
        (struct __vpiSignal *)calloc(alloc_count, sizeof(struct __vpiSignal));
    alloc_index = 0;
#ifdef CHECK_WITH_VALGRIND
    VALGRIND_MAKE_MEM_NOACCESS(alloc_array,
                               alloc_count * sizeof(struct __vpiSignal));
    VALGRIND_CREATE_MEMPOOL(alloc_array, 0, 1);
    signal_pool_count += 1;
    signal_pool = (__vpiSignal **)realloc(
        signal_pool, signal_pool_count * sizeof(__vpiSignal **));
 ...
 * <name>[<index>].
 */
vpiHandle vpip_make_net(const char *name, int msb, int lsb, bool signed_flag,
                        vvp_net_t *node) {
...
static int PV_get_base(struct __vpiPV *rfp) {
  ...
  if (rfp->sbase != 0) {
    ...
    for (int idx = 0; idx < words; idx += 1) {
      ...
      if (val.value.vector[idx].bval != 0)
        return INT_MIN;
    }
  ...
  for (unsigned idx = 0; (idx < rfp->twid) && (idx < 8 * sizeof(tval));
       idx += 1) {
    vvp_bit4_t bit = vthread_get_bit(vpip_current_vthread, rfp->tbase + idx);
    switch (bit) {
    case BIT4_X:
    case BIT4_Z:
...
static vpiHandle PV_put_value(vpiHandle ref, p_vpi_value vp, int) {
  ...
  if (base >= (signed)sig_size)
    return 0;
  if (base + (signed)width < 0)
    return 0;
...
static const struct __vpirt vpip_PV_rt = {
    vpiPartSelect, PV_get,        PV_get_str, PV_get_value,
    PV_put_value,  PV_get_handle, 0};
