...
ufunc_core::ufunc_core(unsigned owid, vvp_net_t *ptr, unsigned nports,
                       ...
                       struct __vpiScope *call_scope__, char *result_label,
                       char *scope_label)
...
void ufunc_core::finish_thread(vthread_t thr) {
  thread_ = 0;
  if (vvp_fun_signal_real *sig =
          dynamic_cast<vvp_fun_signal_real *>(result_->fun))

    propagate_real(sig->real_value());

  if (vvp_fun_signal_vec *sig =
          dynamic_cast<vvp_fun_signal_vec *>(result_->fun))
 ...
 */
void ufunc_core::recv_vec4_from_inputs(unsigned port) { invoke_thread_(); }

void ufunc_core::recv_real_from_inputs(unsigned port) { invoke_thread_(); }
 ...
 */
void compile_ufunc(char *label, char *code, unsigned wid, unsigned argc,
                   struct symb_s *argv, unsigned portc, struct symb_s *portv,
                   struct symb_s retv, char *scope_label, char *trigger_label) {
  ...
  ufunc_core *fcore = new ufunc_core(wid, ptr, portc, ports, start_code,
                                     call_scope, retv.text, scope_label);
  ptr->fun          = fcore;
