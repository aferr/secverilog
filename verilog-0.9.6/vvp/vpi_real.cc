...
static vpiHandle real_var_iterate(int code, vpiHandle ref) {
  ...
  if (code == vpiIndex) {
    return rfp->is_netarray
               ? (rfp->id.index->vpi_type->iterate_)(code, rfp->id.index)
               : 0;
  }
...
static void real_var_get_value(vpiHandle ref, s_vpi_value *vp) {
  ...
  struct __vpiRealVar *rfp = (struct __vpiRealVar *)ref;
  vvp_fun_signal_real *fun = dynamic_cast<vvp_fun_signal_real *>(rfp->net->fun);
...
static const struct __vpirt vpip_real_var_rt = {vpiRealVar,
...
void vpip_real_value_change(struct __vpiCallback *cbh, vpiHandle ref) {
  struct __vpiRealVar *rfp = (struct __vpiRealVar *)ref;
  vvp_fun_signal_real *fun = dynamic_cast<vvp_fun_signal_real *>(rfp->net->fun);
...
vpiHandle vpip_make_real_var(const char *name, vvp_net_t *net) {
  struct __vpiRealVar *obj =
      (struct __vpiRealVar *)malloc(sizeof(struct __vpiRealVar));
