...
static const struct __vpirt vpip_systask_def_rt = {vpiSysTask, 0, 0, 0,
                                                   0,          0, 0};

static const struct __vpirt vpip_sysfunc_def_rt = {vpiSysFunc, 0, 0, 0,
                                                   0,          0, 0};
...
static const struct __vpirt vpip_systask_rt = {
    vpiSysTaskCall, systask_get, systask_get_str, 0, 0,
...
static vpiHandle sysfunc_put_value(vpiHandle ref, p_vpi_value vp, int) {
  ...
  case vpiIntVal: {
    ...
    for (int idx = 0; idx < rfp->vwid; idx += 1) {
      vthread_put_bit(vpip_current_vthread, rfp->vbit + idx,
                      (val & 1) ? BIT4_1 : BIT4_0);
      val >>= 1;
    }
    break;
  ...
  case vpiStringVal: {
    ...
    for (unsigned wdx = 0; wdx < (unsigned)rfp->vwid; wdx += 8) {
      ...
      for (unsigned idx = 0; (wdx + idx) < (unsigned)rfp->vwid && idx < 8;
           idx += 1) {
        vvp_bit4_t bit4 = BIT4_0;
        if (bits & 1)
          bit4 = BIT4_1;
        vthread_put_bit(vpip_current_vthread, rfp->vbit + wdx + idx, bit4);
        bits >>= 1;
      }
    }
    break;
  ...
  case vpiVectorVal:
    ...
    for (unsigned wdx = 0; wdx < (unsigned)rfp->vwid; wdx += 32) {
      ...
      for (unsigned idx = 0; (wdx + idx) < (unsigned)rfp->vwid && idx < 32;
           idx += 1) {
...
static vpiHandle sysfunc_put_4net_value(vpiHandle ref, p_vpi_value vp, int) {
  ...
  case vpiVectorVal:
    ...
    for (unsigned wdx = 0; wdx < vwid; wdx += 32) {
      ...
      for (unsigned idx = 0; (wdx + idx) < vwid && idx < 32; idx += 1) {
        int bit = (aval & 1) | ((bval << 1) & 2);
        vvp_bit4_t bit4;
...
static const struct __vpirt vpip_sysfunc_rt = {
    vpiSysFuncCall,    sysfunc_get,    systask_get_str, 0,
    sysfunc_put_value, systask_handle, systask_iter};

static const struct __vpirt vpip_sysfunc_real_rt = {
    vpiSysFuncCall,         sysfunc_get,    systask_get_str, 0,
    sysfunc_put_real_value, systask_handle, systask_iter};

static const struct __vpirt vpip_sysfunc_4net_rt = {
    vpiSysFuncCall,         sysfunc_get,    systask_get_str, 0,
    sysfunc_put_4net_value, systask_handle, systask_iter};

static const struct __vpirt vpip_sysfunc_rnet_rt = {
    vpiSysFuncCall,         sysfunc_get,    systask_get_str, 0,
    sysfunc_put_rnet_value, systask_handle, systask_iter};
...
static struct __vpiUserSystf *allocate_def(void) {
  if (def_table == 0) {
    def_table =
        (struct __vpiUserSystf **)malloc(sizeof(struct __vpiUserSystf *));

    def_table[0] =
        (struct __vpiUserSystf *)calloc(1, sizeof(struct __vpiUserSystf));
...
static void add_vpi_call_error(vpi_call_error_type type, const char *name,
  ...
  vpi_call_error_lst = (vpi_call_error_p)realloc((void *)vpi_call_error_lst,
...
vpiHandle vpip_build_vpi_call(const char *name, unsigned vbit, int vwid,
                              struct vvp_net_t *fnet, unsigned argc,
                              vpiHandle *argv, long file_idx, long lineno) {
