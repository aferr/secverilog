...
inline vvp_vector4_t::vvp_vector4_t(unsigned size__, vvp_bit4_t val)
  ...
  const static unsigned long init_atable[4] = {WORD_0_ABITS, WORD_1_ABITS,
                                               WORD_Z_ABITS, WORD_X_ABITS};
  const static unsigned long init_btable[4] = {WORD_0_BBITS, WORD_1_BBITS,
...
extern bool vector4_to_value(const vvp_vector4_t &a, int64_t &val,
                             bool is_signed, bool is_arithmetic = true);
...
class vvp_fun_signal_base : public vvp_net_fun_t,
                            public vvp_vpi_callback_wordable {

public:
  vvp_fun_signal_base();
  void recv_long(vvp_net_ptr_t port, long bit);
  void recv_long_pv(vvp_net_ptr_t port, long bit, unsigned base, unsigned wid);
...
protected:
  ...
  virtual void release_pv(vvp_net_ptr_t ptr, bool net, unsigned base,
                          unsigned wid)             = 0;
};
...
public:
  ...
  void release_pv(vvp_net_ptr_t port, bool net, unsigned base, unsigned wid);
