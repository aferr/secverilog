...
void vvp_net_fun_t::operator delete(void *) { assert(0); }
...
void vvp_vector4_t::allocate_words_(unsigned wid, unsigned long inita,
                                    unsigned long initb) {
  if (size_ > BITS_PER_WORD) {
...
vvp_vector4_t::vvp_vector4_t(unsigned size__, double val) : size_(size__) {
  ...
  if (exponent < BITS_PER_WORD - 2) {
    if (is_neg)
      this->invert(); // Invert the bits if negative.
  ...
  if (my_words == 0) {
    ...
    /* Round any fractional part up. */
    if (fraction >= ldexp(0.5, BITS_PER_WORD))
      *this += (int64_t)1;
  }
...
void vvp_vector4_t::set_vec(unsigned adr, const vvp_vector4_t &that) {
  ...
  if (size_ <= BITS_PER_WORD) {
    ...
    abits_val_ = (abits_val_ & ~mask) | ((that.abits_val_ << adr) & mask);
    bbits_val_ = (bbits_val_ & ~mask) | ((that.bbits_val_ << adr) & mask);
    ...
    abits_ptr_[dptr] =
        (abits_ptr_[dptr] & ~mask) | ((that.abits_val_ << doff) & mask);
    bbits_ptr_[dptr] =
        (bbits_ptr_[dptr] & ~mask) | ((that.bbits_val_ << doff) & mask);
    ...
    if (remain > 0) {
      ...
      abits_ptr_[dptr] =
          (abits_ptr_[dptr] & ~mask) | (that.abits_ptr_[sptr] & mask);
      bbits_ptr_[dptr] =
          (bbits_ptr_[dptr] & ~mask) | (that.bbits_ptr_[sptr] & mask);
    }
    ...
    while (remain >= BITS_PER_WORD) {
      abits_ptr_[dptr] = (abits_ptr_[dptr] & lmask) |
                         ((that.abits_ptr_[sptr] << doff) & ~lmask);
      bbits_ptr_[dptr] = (bbits_ptr_[dptr] & lmask) |
    ...
    if (remain > 0) {
      ...
      abits_ptr_[dptr] =
          (abits_ptr_[dptr] & ~mask) | ((that.abits_ptr_[sptr] << doff) & mask);
      bbits_ptr_[dptr] =
          (bbits_ptr_[dptr] & ~mask) | ((that.bbits_ptr_[sptr] << doff) & mask);
...
bool vector4_to_value(const vvp_vector4_t &vec, int64_t &val, bool is_signed,
                      bool is_arithmetic) {
  long res     = 0;
  long msk     = 1;
  bool rc_flag = true;
...
vvp_realarray_t::vvp_realarray_t(unsigned wor) : words_(wor) {
...
vvp_realarray_t::~vvp_realarray_t() { delete[] array_; }
...
vvp_vector2_t operator*(const vvp_vector2_t &a, const vvp_vector2_t &b) {
  ...
  for (unsigned bdx = 0; bdx < bwords; bdx += 1) {
    ...
    for (unsigned adx = 0; adx < awords; adx += 1) {
      ...
      unsigned long carry = 0;
      for (unsigned sdx = 0; (adx + bdx + sdx) < rwords; sdx += 1) {
...
vvp_vector8_t &vvp_vector8_t::operator=(const vvp_vector8_t &that) {
  // Assign to self.
  if (this == &that ||
      (size_ > PTR_THRESH && that.size_ > PTR_THRESH && ptr_ == that.ptr_))
    return *this;
...
vvp_vector8_t vvp_vector8_t::subvalue(unsigned base, unsigned wid) const {
  ...
  vvp_scalar_t *tmp_ptr = tmp.size_ <= PTR_THRESH
                              ? reinterpret_cast<vvp_scalar_t *>(tmp.val_)
                              : tmp.ptr_;
  const vvp_scalar_t *ptr =
      size_ <= PTR_THRESH ? reinterpret_cast<const vvp_scalar_t *>(val_) : ptr_;
...
vvp_vector8_t part_expand(const vvp_vector8_t &that, unsigned wid,
  ...
  vvp_scalar_t *tmp_ptr = tmp.size_ <= vvp_vector8_t::PTR_THRESH
                              ? reinterpret_cast<vvp_scalar_t *>(tmp.val_)
                              : tmp.ptr_;
  const vvp_scalar_t *that_ptr =
      that.size_ <= vvp_vector8_t::PTR_THRESH
          ? reinterpret_cast<const vvp_scalar_t *>(that.val_)
          : that.ptr_;
  ...
  case 0: // Normal input
    if (assign_mask_.size() == 0) {
      ...
      for (unsigned idx = 0; idx < wid; idx += 1) {
        if (base + idx >= bits4_.size())
          break;
        if (assign_mask_.value(base + idx))
          continue;
        bits4_.set_bit(base + idx, bit.value(idx));
        changed = true;
      }
...
void vvp_fun_signal4_sa::release_pv(vvp_net_ptr_t ptr, bool net, unsigned base,
                                    unsigned wid) {
  // If not currently forced, do nothing.
  if (force_mask_.size() == 0)
    return;
  ...
  for (unsigned idx = 0; idx < wid; idx += 1) {
    if (force_mask_.value(base + idx)) {
      ...
      if (!net)
        bits4_.set_bit(base + idx, force_.value(base + idx));
    }
  }
...
void vvp_fun_signal4_aa::release_pv(vvp_net_ptr_t ptr, bool net, unsigned base,
                                    unsigned wid) {
  /* Automatic variables can't be forced. */
  assert(0);
}

unsigned vvp_fun_signal4_aa::size() const { return size_; }
...
vvp_fun_signal8::vvp_fun_signal8(unsigned wid) : bits8_(wid) {}
...
void vvp_fun_signal8::release_pv(vvp_net_ptr_t ptr, bool net, unsigned base,
                                 unsigned wid) {
  // If not currently forced, do nothing.
  if (force_mask_.size() == 0)
    return;
  ...
  for (unsigned idx = 0; idx < wid; idx += 1) {
    if (force_mask_.value(base + idx)) {
      force_mask_.set_bit(base + idx, 0);
      if (!net)
        bits8_.set_bit(base + idx, force_.value(base + idx));
    }
  }
  if (force_mask_.is_zero())
    force_mask_ = vvp_vector2_t();
...
void vvp_wide_fun_core::recv_real_from_inputs(unsigned p) { assert(0); }
...
void vvp_wide_fun_core::dispatch_real_from_input_(unsigned port, double bit) {
  assert(port < nports_);
  if (port_rvalues_ == 0)
    port_rvalues_ = new double[nports_];
...
vvp_vector8_t resistive_reduction(const vvp_vector8_t &that) {
  ...
  for (unsigned idx = 0; idx < res.size(); idx += 1) {
    ...
    bit =
        vvp_scalar_t(bit.value(), rstr[bit.strength0()], rstr[bit.strength1()]);
    res.set_bit(idx, bit);
  }
