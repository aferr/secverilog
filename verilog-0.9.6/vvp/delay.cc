...
vvp_fun_delay::vvp_fun_delay(vvp_net_t *n, vvp_bit4_t init,
                             const vvp_delay_t &d)
...
bool vvp_fun_delay::clean_pulse_events_(vvp_time64_t use_delay,
  ...
  if (list_->next->ptr_vec4.eeq(bit))
    return true;
...
bool vvp_fun_delay::clean_pulse_events_(vvp_time64_t use_delay,
  ...
  /* If the most recent event and the new event have the same
   * value then we need to skip the new event. */
  if (list_->next->ptr_vec8.eeq(bit))
    return true;
...
void vvp_fun_delay::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t &bit,
  ...
  /* what *should* happen here is we check to see if there is a
     ...
     eliminated. */
  if (clean_pulse_events_(use_delay, bit))
    return;
...
void vvp_fun_modpath::recv_vec4(vvp_net_ptr_t port, const vvp_vector4_t &bit,
  ...
  for (unsigned idx = 1; idx < bit.size(); idx += 1) {
    vvp_time64_t tmp =
        delay_from_edge(cur_vec4_.value(idx), bit.value(0), out_at);
...
void vvp_fun_modpath::run_run() { vvp_send_vec4(net_->out, cur_vec4_, 0); }
 ...
 */
static const struct __vpirt vpip_modpath_src_rt = {vpiModPath,
                                                   modpath_src_get,
                                                   0, /* vpi_get_str */
...
struct __vpiModPath *vpip_make_modpath(vvp_net_t *net) {
  struct __vpiModPath *obj =
      (struct __vpiModPath *)calloc(1, sizeof(struct __vpiModPath));
...
struct __vpiModPathSrc *vpip_make_modpath_src(struct __vpiModPath *path,
  ...
  struct __vpiModPathSrc *obj =
      (struct __vpiModPathSrc *)calloc(1, sizeof(struct __vpiModPathSrc));
