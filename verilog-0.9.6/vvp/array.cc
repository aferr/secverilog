...
static const struct __vpirt vpip_arraymem_rt = {
    vpiMemory, vpi_array_get,        vpi_array_get_str, 0,
    0,         vpi_array_get_handle, vpi_array_iterate, vpi_array_index,
};
...
static const struct __vpirt vpip_array_index_rt = {vpiIterator,
                                                   0,
                                                   0,
                                                   0,
                                                   0,
...
static const struct __vpirt vpip_array_var_index_rt = {
    vpiIndex, 0, 0, &vpi_array_var_index_get_value, 0, 0, 0, 0, 0};
...
#define ARRAY_HANDLE(ref)                                                      \
  (assert(ref->vpi_type->type_code == vpiMemory), (struct __vpiArray *)ref)
...
static vpiHandle array_iterator_scan(vpiHandle ref, int) {
  ...
  if (obj->array->nets)
    return obj->array->nets[use_index];
...
static unsigned vpi_array_is_real(vvp_array_t arr) {
  ...
  if (arr->valsr != 0)
    return 1U; // A real variable array.
  if (arr->vals4 != 0)
    return 0U; // A bit based variable/register array.
...
static vpiHandle vpi_array_vthr_A_put_value(vpiHandle ref, p_vpi_value vp,
                                            int) {
  struct __vpiArrayVthrA *obj = array_vthr_a_from_handle(ref);
  assert(obj);
  struct __vpiArray *parent = obj->array;
...
void array_set_word(vvp_array_t arr, unsigned address, unsigned part_off,
                    vvp_vector4_t val) {
...
vvp_vector4_t array_get_word(vvp_array_t arr, unsigned address) {
  ...
  if (address >= arr->array_count) {
    ...
    vvp_fun_signal_vec *sig =
        dynamic_cast<vvp_fun_signal_vec *>(vsig->node->fun);
    assert(sig);
...
double array_get_word_r(vvp_array_t arr, unsigned address) {
  ...
  assert(vsig);
  vvp_fun_signal_real *sig =
      dynamic_cast<vvp_fun_signal_real *>(vsig->net->fun);
  assert(sig);
...
static vpiHandle vpip_make_array(char *label, const char *name, int first_addr,
                                 int last_addr, bool signed_flag) {
  struct __vpiArray *obj =
      (struct __vpiArray *)malloc(sizeof(struct __vpiArray));
...
void compile_var_array(char *label, char *name, int last, int first, int msb,
  ...
  if (vpip_peek_current_scope()->is_automatic) {
    arr->vals4 = new vvp_vector4array_aa(arr->vals_width, arr->array_count);
  } else {
    arr->vals4 = new vvp_vector4array_sa(arr->vals_width, arr->array_count);
  }
...
void compile_real_array(char *label, char *name, int last, int first, int msb,
                        int lsb) {
...
void vvp_fun_arrayport_aa::recv_vec4(vvp_net_ptr_t port,
  ...
  if (context) {
    ...
    case 0: // Address input
      ...
      if (vpi_array_is_real(arr_)) {
        vvp_send_real(port.ptr()->out, array_get_word_r(arr_, *addr), context);
      } else {
        vvp_send_vec4(port.ptr()->out, array_get_word(arr_, *addr), context);
      }
      break;
...
void array_word_change(vvp_array_t array, unsigned long addr) {
  ...
  while (next) {
    ...
    if (cur->extra_data == -1) {
      cur->cb_data.index = (PLI_INT32)((int)addr + array->first_addr.value);
    }

    if (cur->cb_data.cb_rtn != 0) {
      if (cur->cb_data.value) {
        if (vpi_array_is_real(array)) {
          vpip_real_get_value(array->valsr->get_word(addr), cur->cb_data.value);
        } else {
          vpip_vec4_get_value(array->vals4->get_word(addr), array->vals_width,
...
void compile_array_port(char *label, char *array, char *addr) {
  array_port_resolv_list_t *resolv_mem = new array_port_resolv_list_t(array);
...
vpiHandle vpip_make_vthr_A(char *label, unsigned addr) {
  struct __vpiArrayVthrA *obj =
      (struct __vpiArrayVthrA *)malloc(sizeof(struct __vpiArrayVthrA));
...
void memory_delete(vpiHandle item) {
  ...
  if (arr->nets) {
    for (unsigned idx = 0; idx < arr->array_count; idx += 1) {
      if (struct __vpiSignal *sig = vpip_signal_from_handle(arr->nets[idx])) {
        // Delete the individual words?
        constant_delete(sig->id.index);
        /* These should only be the real words. */
      } else {
        assert(arr->nets[idx]->vpi_type->type_code == vpiRealVar);
