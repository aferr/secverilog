...
bool vpi_handle_resolv_list_s::resolve(bool mes) {
  ...
  if (!val.ptr) {
    ...
    } else if (3 <= sscanf(label(), "T<%u,%u,%[su]>%u", &base, &wid, ss, &n) &&
               n == strlen(label())) {

      bool signed_flag = false;
      for (char *fp = ss; *fp; fp += 1)
        switch (*fp) {
        case 's':
          signed_flag = true;
          break;
...
void input_connect(vvp_net_t *fdx, unsigned port, char *label) {
  ...
  if ((strncmp(label, "C4<", 3) == 0) && ((tp = strchr(label, '>'))) &&
      (tp[1] == 0) &&
  ...
  if ((strncmp(label, "C8<", 3) == 0) && ((tp = strchr(label, '>'))) &&
      (tp[1] == 0) &&
  ...
  /* Handle the Cr<> constant driver, which is a real-value
     driver. */
  if ((strncmp(label, "Cr<", 3) == 0) && ((tp = strchr(label, '>'))) &&
      (tp[1] == 0) &&
...
void compile_modpath_src(struct __vpiModPath *dst, char edge, struct symb_s src,
                         struct numbv_s vals, struct symb_s condit_src,
...
void compile_modpath_src(struct __vpiModPath *dst, char edge, struct symb_s src,
                         struct numbv_s vals, int condit_src,
                         struct symb_s path_term_in, bool ifnone) {
  assert(condit_src == 0);
...
void compile_vpi_call(char *label, char *name, long file_idx, long lineno,
  ...
  code->handle =
      vpip_build_vpi_call(name, 0, 0, 0, argc, argv, file_idx, lineno);
  if (code->handle == 0)
    compile_errors += 1;
...
void compile_vpi_func_call(char *label, char *name, unsigned vbit, int vwid,
  ...
  /* Create a vpiHandle that bundles the call information, and
     store that handle in the instruction. */
  code->handle =
      vpip_build_vpi_call(name, vbit, vwid, 0, argc, argv, file_idx, lineno);
  if (code->handle == 0)
    compile_errors += 1;
...
void compile_param_logic(char *label, char *name, char *value, bool signed_flag,
  ...
  vpiHandle obj =
      vpip_make_binary_param(name, value4, signed_flag, file_idx, lineno);
...
void compile_param_string(char *label, char *name, char *value, long file_idx,
                          long lineno) {
...
void compile_param_real(char *label, char *name, char *value, long file_idx,
                        long lineno) {
