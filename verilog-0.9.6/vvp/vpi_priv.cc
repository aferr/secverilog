...
unsigned vpip_size(__vpiSignal *sig) { return abs(sig->msb - sig->lsb) + 1; }
  ...
  if (property == vpiType) {
    if (vpi_trace) {
      fprintf(vpi_trace, "vpi_get(vpiType, %p) --> %s\n", (void *)ref,
              vpi_type_values(ref->vpi_type->type_code));
    }
  ...
  if (type == vpiSysTfCall) {
    ...
    if (vpi_trace) {
      fprintf(vpi_trace,
              ...
              "-> %p (%s)\n",
              (void *)&vpip_cur_task->base, vpip_cur_task->defn->info.tfname);
    }
