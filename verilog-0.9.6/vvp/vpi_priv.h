 ...
 */
template <class T> vpiHandle vpi_handle(T obj) { return &obj->base; }
...
extern void vpip_part_select_value_change(struct __vpiCallback *cbh,
                                          vpiHandle obj);
...
extern struct __vpiModPathSrc *
vpip_make_modpath_src(struct __vpiModPath *path_dest,
                      vvp_time64_t use_delay[12], vvp_net_t *net);
...
extern void vpip_real_value_change(struct __vpiCallback *cbh, vpiHandle ref);
...
vpiHandle vpip_make_string_param(char *name, char *value, long file_idx,
                                 long lineno);
 ...
 */
extern vpiHandle vpip_build_vpi_call(const char *name, unsigned vbit, int vwid,
                                     struct vvp_net_t *fnet, unsigned argc,
                                     vpiHandle *argv, long file_idx,
                                     long lineno);
...
extern vvp_time64_t vpip_scaled_real_to_time64(double val,
                                               struct __vpiScope *sc);
