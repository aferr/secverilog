...
void sfunc_core::recv_vec4_from_inputs(unsigned port) {
  ...
  struct __vpiBinaryConst *obj = (struct __vpiBinaryConst *)vpi;
...
void sfunc_core::recv_real_from_inputs(unsigned port) {
  ...
  struct __vpiRealConst *obj = (struct __vpiRealConst *)vpi;
...
void compile_sfunc(char *label, char *name, char *format_string, long file_idx,
  ...
  vpiHandle sys = vpip_build_vpi_call(name, 0, width_code, ptr, argc, vpi_argv,
                                      file_idx, lineno);
  assert(sys);
