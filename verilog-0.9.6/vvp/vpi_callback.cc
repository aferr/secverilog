 ...
 */
static int free_simple_callback(vpiHandle ref) { return 1; }

const struct __vpirt callback_rt = {vpiCallback,
...
void callback_execute(struct __vpiCallback *cur) {
  ...
  case vpiScaledRealTime: {
    cur->cb_data.time->real = vpip_time_to_scaled_real(
        schedule_simtime(),
        (struct __vpiScope *)vpi_handle(vpiScope, cur->cb_data.obj));
    break;
  }
  case vpiSuppressTime:
    break;
...
vvp_vpi_callback::vvp_vpi_callback() { vpi_callbacks_ = 0; }

vvp_vpi_callback::~vvp_vpi_callback() { assert(vpi_callbacks_ == 0); }
...
vvp_vpi_callback_wordable::~vvp_vpi_callback_wordable() { assert(array_ == 0); }

void vvp_vpi_callback_wordable::run_vpi_callbacks() {
  if (array_)
    array_word_change(array_, array_word_);

  vvp_vpi_callback::run_vpi_callbacks();
}

void vvp_vpi_callback_wordable::attach_as_word(vvp_array_t arr,
                                               unsigned long addr) {
  assert(array_ == 0);
  array_      = arr;
  array_word_ = addr;
