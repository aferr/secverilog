...
NetExpr *NetExpr::eval_tree(int prune_to_width) { return 0; }
...
bool NetEBinary::get_real_arguments_(verireal &lval, verireal &rval) {
  if (!get_real_arg_(left_, lval))
    return false;
  if (!get_real_arg_(right_, rval))
    return false;
...
NetEConst *NetEBComp::must_be_leeq_(NetExpr *le, const verinum &rv,
                                    bool eq_flag) {
...
NetEConst *NetEBLogic::eval_tree(int prune_to_width) {
  ...
  if (debug_eval_tree)
    cerr << get_fileline() << ": debug: Evaluated: " << *this << " --> " << *tmp
         << endl;
...
NetExpr *NetEBMult::eval_tree(int prune_to_width) {
  ...
  if (debug_eval_tree)
    cerr << get_fileline() << ": debug: Evaluate " << lval << " * " << rval
         << " --> " << *tmp << endl;
...
NetExpr *NetEBPow::eval_tree_real_() {
  ...
  if (debug_eval_tree)
    cerr << get_fileline() << ": debug: Evaluate (real) " << lval << " ** "
         << rval << " --> " << *res << endl;
...
NetExpr *NetEParam::eval_tree(int prune_to_width) {
  ...
  if (debug_eval_tree) {
    cerr << get_fileline() << ": debug: evaluating expression: " << *this
         << endl;
  }

  if (solving()) {
    cerr << get_fileline()
         << ": warning: Recursive parameter "
            "reference found involving "
         << *this << "." << endl;
    return 0;
  }
  ...
  if (expr == NULL) {
    cerr << get_fileline() << ": internal error: parameter/localparam " << *this
  ...
  if (res == 0) {
    ...
    if (expr_type() == IVL_VT_REAL)
      cerr << "real ";
    cerr << "parameter " << name << " expression: " << *nexpr << endl;
    delete nexpr;
    return 0;
  }
  ...
  case IVL_VT_LOGIC: {
    ...
    if (tmp == 0) {
      cerr << get_fileline() << ": internal error: parameter " << name
           << " evaluates to incomprehensible " << *res << "." << endl;
      return 0;
    }
...
NetExpr *NetETernary::eval_tree(int prune_to_width) {
  ...
  case C_0:
    ...
    if (debug_eval_tree) {
      ...
      cerr << get_fileline() << ":      : Selecting false case: " << *false_val_
           << endl;
    }

    if (expr_type() == IVL_VT_REAL && false_val_->expr_type() != IVL_VT_REAL) {
      verireal f;
  ...
  if (t == 0 || f == 0) {
    verireal tv, fv;
    if (!get_real_arg_(true_val_, tv))
      return 0;
    if (!get_real_arg_(false_val_, fv))
      return 0;

    verireal val = verireal(0.0);
    if (tv.as_double() == fv.as_double())
      val = tv;
...
NetExpr *evaluate_math_two_args(NetExpr *&arg0_, NetExpr *&arg1_,
                                const char *name) {
  eval_expr(arg0_);
  eval_expr(arg1_);
...
NetExpr *NetESFunc::eval_tree(int prune_to_width) {
  ...
  if (generation_flag < GN_VER2005 && !gn_icarus_misc_flag &&
      !gn_verilog_ams_flag) {
    return 0;
  }
  ...
  if (strcmp(nm, "$clog2") == 0 || strcmp(nm, "$ln") == 0 ||
      strcmp(nm, "$log10") == 0 || strcmp(nm, "$exp") == 0 ||
      strcmp(nm, "$sqrt") == 0 || strcmp(nm, "$floor") == 0 ||
      strcmp(nm, "$ceil") == 0 || strcmp(nm, "$sin") == 0 ||
      strcmp(nm, "$cos") == 0 || strcmp(nm, "$tan") == 0 ||
      strcmp(nm, "$asin") == 0 || strcmp(nm, "$acos") == 0 ||
      strcmp(nm, "$atan") == 0 || strcmp(nm, "$sinh") == 0 ||
      strcmp(nm, "$cosh") == 0 || strcmp(nm, "$tanh") == 0 ||
      strcmp(nm, "$asinh") == 0 || strcmp(nm, "$acosh") == 0 ||
      strcmp(nm, "$atanh") == 0) {
    if (nparms() != 1 || parm(0) == 0) {
      cerr << get_fileline() << ": error: " << nm << " takes a single argument."
           << endl;
      return 0;
    }
  ...
  if (strcmp(nm, "$pow") == 0 || strcmp(nm, "$atan2") == 0 ||
      strcmp(nm, "$hypot") == 0) {
    if (nparms() != 2 || parm(0) == 0 || parm(1) == 0) {
      cerr << get_fileline() << ": error: " << nm << " takes two arguments."
           << endl;
      return 0;
    }
  ...
  if (rtn != 0) {
    rtn->set_line(*this);
    if (debug_eval_tree) {
      cerr << get_fileline() << ": debug: Evaluate constant " << nm << "."
           << endl;
    }
  }
