...
list<NetScope *> Design::find_root_scopes() { return root_scopes_; }

const list<NetScope *> Design::find_root_scopes() const { return root_scopes_; }
...
void NetScope::evaluate_parameter_logic_(Design *des, param_ref_t cur) {
  ...
  if (expr == NULL)
    return; // This is an invalid parameter so return.
  ...
  if (range_flag) {
    ...
    if (!(value.has_len() && (value.len() == wid) &&
...
void NetScope::evaluate_parameter_real_(Design *des, param_ref_t cur) {
  ...
  case IVL_VT_BOOL:
    if (NetEConst *tmp = dynamic_cast<NetEConst *>(expr)) {
      ...
      cerr << expr->get_fileline() << ": error: "
           << "Unable to evaluate parameter " << (*cur).first
           << " value: " << *expr << endl;
      des->errors += 1;
      (*cur).second.expr = NULL;
      return;
