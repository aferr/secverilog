...
static void usage() {
  fprintf(stderr,
          "usage: iverilog-vpi" IVERILOG_SUFFIX " [src and obj files]...\n");
...
static int GetRegistryKey(char *key, char **value) {
  ...
  lrv = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Icarus Verilog", 0,
...
static void SetRegistryKey(char *key, char *value) {
  ...
  if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, "Software\\Icarus Verilog", 0, "",
...
static void checkIvlDir(char *root) {
  ...
  if (irv) {
    fprintf(stderr,
            ...
            root);
    fprintf(stderr, "       Icarus Verilog.  Use the -ivl option of "
                    "iverilog-vpi" IVERILOG_SUFFIX " to\n");
    fprintf(
        stderr,
        "       point to the Icarus Verilog root directory.  For a Windows\n");
    fprintf(stderr, "       command shell the option would be something like "
                    "-ivl=c:\\iverilog\n");
    fprintf(stderr,
            "       For a Cygwin shell the option would be something like\n");
...
static void setup_mingw_environment() {
  ...
  if (*gstr.pMINGW) {
    ...
    fprintf(stderr, "error: can not locate the MinGW root directory, use the "
                    "-mingw option of\n");
    fprintf(
        stderr,
...
static void setup_ivl_environment() {
  if (*gstr.pIVL) {
    ...
    fprintf(stderr, "error: can not locate the Icarus Verilog root directory, "
                    "use the -ivl option\n");
    fprintf(stderr, "       of iverilog-vpi" IVERILOG_SUFFIX
                    " to point to the Icarus Verilog root directory.\n");
    fprintf(stderr, "       For a Windows command shell the option would be "
                    "something like\n");
    fprintf(stderr, "       -ivl=c:\\iverilog  For a Cygwin shell the option "
                    "would be something\n");
...
static void compile(char *pSource, char **pObject, int *compile_errors,
                    char *compiler) {
  char *ptr1 = pSource;
