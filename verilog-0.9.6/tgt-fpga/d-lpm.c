...
static void lpm_show_footer(ivl_design_t des) { edif_print(xnf, edf); }
...
static void lpm_show_dff(ivl_lpm_t net) {
  ...
  sprintf(name, "fd%s%s%s%s%s%u", ivl_lpm_enable(net) ? "ce" : "",
          ivl_lpm_async_clr(net) ? "cl" : "", ivl_lpm_sync_clr(net) ? "sc" : "",
          ivl_lpm_async_set(net) ? "se" : "", ivl_lpm_sync_set(net) ? "ss" : "",
          wid);
  ...
  if (cell == 0) {
    ...
    for (idx = 0; idx < wid; idx += 1) {
      ...
      edif_cell_portconfig(cell, idx * 2 + 0, strdup(name), IVL_SIP_OUTPUT);

      sprintf(name, "Data%u", idx);
      edif_cell_portconfig(cell, idx * 2 + 1, strdup(name), IVL_SIP_INPUT);
    }
...
static void lpm_show_mux(ivl_lpm_t net) {
  ...
  if (cell == 0) {
    ...
    for (idx = 0; idx < wid_r; idx += 1) {
      ...
      edif_cell_portconfig(cell, idx, strdup(cellname), IVL_SIP_OUTPUT);
    }

    /* Make the select ports. */
    for (idx = 0; idx < wid_s; idx += 1) {
      sprintf(cellname, "Sel%u", idx);
      edif_cell_portconfig(cell, wid_r + idx, strdup(cellname), IVL_SIP_INPUT);
    }
    ...
    for (idx = 0; idx < wid_z; idx += 1) {
      ...
      for (rdx = 0; rdx < wid_r; rdx += 1) {
        ...
        edif_cell_portconfig(cell, base + rdx, strdup(cellname), IVL_SIP_INPUT);
      }
    }
...
static void lpm_show_add(ivl_lpm_t net) {
  ...
  if (cell == 0) {
    ...
    for (idx = 0; idx < cell_width; idx += 1) {
      ...
      sprintf(cellname, "Result%u", idx);
      edif_cell_portconfig(cell, idx * 3 + 0, strdup(cellname), IVL_SIP_OUTPUT);

      sprintf(cellname, "DataA%u", idx);
      edif_cell_portconfig(cell, idx * 3 + 1, strdup(cellname), IVL_SIP_INPUT);

      sprintf(cellname, "DataB%u", idx);
      edif_cell_portconfig(cell, idx * 3 + 2, strdup(cellname), IVL_SIP_INPUT);
    }
...
static void lpm_show_mult(ivl_lpm_t net) {
  ...
  if (cell == 0) {
    cell = edif_xcell_create(xlib, strdup(name), 3 * ivl_lpm_width(net));

    for (idx = 0; idx < ivl_lpm_width(net); idx += 1) {

      sprintf(name, "Result%u", idx);
      edif_cell_portconfig(cell, idx * 3 + 0, strdup(name), IVL_SIP_OUTPUT);

      sprintf(name, "DataA%u", idx);
      edif_cell_portconfig(cell, idx * 3 + 1, strdup(name), IVL_SIP_INPUT);

      sprintf(name, "DataB%u", idx);
      edif_cell_portconfig(cell, idx * 3 + 2, strdup(name), IVL_SIP_INPUT);
    }
