...
static void edif_show_consts(ivl_design_t des) {
  ...
  for (idx = 0; idx < ivl_design_consts(des); idx += 1) {
    ...
    for (pin = 0; pin < ivl_const_pins(net); pin += 1) {
      ...
      sprintf(jbuf, "(portRef %s (instanceRef U%u))", port, edif_uref);
...
static void edif_show_footer(ivl_design_t des) {
  ...
  for (cur = net_list; cur; cur = cur->next) {
    fprintf(xnf, "(net (rename N%u \"%s\") (joined %s))\n", nref,
            ivl_nexus_name(cur->nex), cur->joined);
    nref += 1;
  }
...
static void edif_show_logic(ivl_net_logic_t net) {
  ...
  case IVL_LO_AND:
    ...
    fprintf(xnf, "(instance (rename U%u \"%s\")", edif_uref,
            ivl_logic_name(net));
    fprintf(xnf,
            " (viewRef net"
    ...
    for (idx = 1; idx < ivl_logic_pins(net); idx += 1) {
      sprintf(jbuf, "(portRef I%u (instanceRef U%u))", idx - 1, edif_uref);
  ...
  case IVL_LO_BUFZ: {
    ...
    if (!bufz_warned_once) {
      fprintf(stderr, "0:0: internal warning: BUFZ objects found "
                      "in EDIF netlist.\n");
...
static void edif_show_generic_dff(ivl_lpm_t net) {
  ...
  for (idx = 0; idx < ivl_lpm_width(net); idx += 1) {
    ...
    fprintf(xnf, "(instance (rename U%u \"%s.%s[%u]\")", edif_uref,
            ivl_scope_name(ivl_lpm_scope(net)), ivl_lpm_basename(net), idx);
    fprintf(xnf,
            " (viewRef net"
            " (cellRef %s (libraryRef VIRTEX))))\n",
            fdcell);
    ...
    if (aset) {
      if (abits[idx] == '1') {
        sprintf(jbuf, "(portRef PRE (instanceRef U%u))", edif_uref);
