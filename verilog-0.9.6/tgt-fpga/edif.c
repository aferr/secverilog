...
typedef enum property_e { PRP_NONE = 0, PRP_STRING, PRP_INTEGER } property_t;
...
void edif_portconfig(edif_t edf, unsigned idx, const char *name,
                     ivl_signal_port_t dir) {
...
edif_cell_t edif_xlibrary_scope_cell(edif_xlibrary_t xlib, ivl_scope_t scope) {
  ...
  for (idx = 0; idx < ivl_scope_sigs(scope); idx += 1) {
    ...
    edif_cell_portconfig(cur, port_count, ivl_signal_basename(sig),
                         ivl_signal_port(sig));
    port_count += 1;
  }
...
void edif_cell_portconfig(edif_cell_t cell, unsigned idx, const char *name,
                          ivl_signal_port_t dir) {
...
void edif_cell_port_pstring(edif_cell_t cell, unsigned idx, const char *name,
                            const char *value) {
  struct cellref_property_ *prp = malloc(sizeof(struct cellref_property_));
  prp->name                     = name;
  prp->ptype                    = PRP_STRING;
...
void edif_cell_pstring(edif_cell_t cell, const char *name, const char *value) {
  struct cellref_property_ *prp = malloc(sizeof(struct cellref_property_));
  prp->name                     = name;
  prp->ptype                    = PRP_STRING;
  prp->value_.str               = value;
...
void edif_cell_pinteger(edif_cell_t cell, const char *name, int value) {
  struct cellref_property_ *prp = malloc(sizeof(struct cellref_property_));
  prp->name                     = name;
...
void edif_print(FILE *fd, edif_t edf) {
  ...
  for (idx = 0; idx < edf->nports; idx += 1) {
    ...
    if (edf->ports[idx].ename == 0)
      ...
      fprintf(fd, "(rename %s \"%s\") ", edf->ports[idx].ename,
              edf->ports[idx].name);
    ...
    for (jc = jnt->links; jc; jc = jc->next) {
      if (jc->cell) {
        fprintf(fd, " (portRef %s (instanceRef U%u))",
                jc->cell->cell->ports[jc->port].name, jc->cell->u);
      } else {
        /* Reference to a port of the main cell. */
        if (edf->ports[jc->port].ename)
          fprintf(fd, " (portRef %s)", edf->ports[jc->port].ename);
        else
          fprintf(fd, " (portRef %s)", edf->ports[jc->port].name);
      }
    }
    fprintf(fd, "))\n");
  }
